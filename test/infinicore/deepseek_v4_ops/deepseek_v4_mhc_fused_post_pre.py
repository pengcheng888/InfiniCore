import argparse
import os
import sys

import torch

sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

import infinicore
from infinicore.lib import _infinicore


DEFAULT_TOKENS = "1,2,4,8,16,32,64,128,256,512,1024,2048,4096,8192"


def _parse_tokens(text):
    return [int(item) for item in text.split(",") if item.strip()]


def _as_core(tensor):
    return infinicore.from_torch(tensor)


def _make_inputs(tokens, hc, hidden, device):
    dtype = torch.bfloat16
    mix_hc = (2 + hc) * hc
    k = hc * hidden

    x = torch.randn((tokens, hidden), device=device, dtype=dtype).contiguous()
    residual = torch.randn((tokens, hc, hidden), device=device, dtype=dtype).contiguous()
    post_layer_mix = (torch.rand((tokens, hc), device=device, dtype=torch.float32) * 2.0).contiguous()
    comb_res_mix = torch.rand((tokens, hc, hc), device=device, dtype=torch.float32)
    comb_res_mix = (comb_res_mix / (comb_res_mix.sum(dim=1, keepdim=True) + 1e-6)).contiguous()
    fn = (torch.randn((mix_hc, k), device=device, dtype=torch.float32) * 0.02).contiguous()
    hc_scale = (torch.randn((3,), device=device, dtype=torch.float32) * 0.1).contiguous()
    hc_base = (torch.randn((mix_hc,), device=device, dtype=torch.float32) * 0.1).contiguous()
    norm_weight = (torch.randn((hidden,), device=device, dtype=dtype) * 0.1).contiguous()
    return x, residual, post_layer_mix, comb_res_mix, fn, hc_scale, hc_base, norm_weight


def _make_outputs(tokens, hc, hidden, device):
    residual = torch.empty((tokens, hc, hidden), device=device, dtype=torch.bfloat16)
    post = torch.empty((tokens, hc), device=device, dtype=torch.float32)
    comb = torch.empty((tokens, hc, hc), device=device, dtype=torch.float32)
    layer_input = torch.empty((tokens, hidden), device=device, dtype=torch.bfloat16)
    return residual, post, comb, layer_input


def _ref_separate(
    core_residual_out,
    core_post_out,
    core_comb_out,
    core_layer_input_out,
    core_x,
    core_residual,
    core_post_layer_mix,
    core_comb_res_mix,
    core_fn,
    core_hc_scale,
    core_hc_base,
    core_norm_weight,
    tensors,
    args,
):
    _infinicore.deepseek_v4_mhc_post_kernel_(
        core_residual_out._underlying,
        core_x._underlying,
        core_residual._underlying,
        core_post_layer_mix._underlying,
        core_comb_res_mix._underlying,
    )
    _infinicore.deepseek_v4_mhc_pre_kernel_(
        core_layer_input_out._underlying,
        core_post_out._underlying,
        core_comb_out._underlying,
        core_residual_out._underlying,
        core_fn._underlying,
        core_hc_scale._underlying,
        core_hc_base._underlying,
        args.rms_eps,
        args.hc_pre_eps,
        args.hc_sinkhorn_eps,
        args.sinkhorn_repeat,
    )
    _infinicore.rms_norm_(
        core_layer_input_out._underlying,
        core_layer_input_out._underlying,
        core_norm_weight._underlying,
        args.norm_eps,
    )
    return tensors


def _max_diff(lhs, rhs):
    lhs_f = lhs.float()
    rhs_f = rhs.float()
    abs_diff = (lhs_f - rhs_f).abs()
    max_abs = abs_diff.max().item() if abs_diff.numel() > 0 else 0.0
    denom = rhs_f.abs().clamp_min(1e-6)
    max_rel = (abs_diff / denom).max().item() if abs_diff.numel() > 0 else 0.0
    return max_abs, max_rel


def _tuple_max_diff(lhs_tensors, rhs_tensors):
    max_abs = 0.0
    max_rel = 0.0
    for lhs, rhs in zip(lhs_tensors, rhs_tensors):
        abs_diff, rel_diff = _max_diff(lhs, rhs)
        max_abs = max(max_abs, abs_diff)
        max_rel = max(max_rel, rel_diff)
    return max_abs, max_rel


def _tuple_allclose(lhs_tensors, rhs_tensors, atol, rtol):
    return all(
        torch.allclose(lhs.float(), rhs.float(), atol=atol, rtol=rtol)
        for lhs, rhs in zip(lhs_tensors, rhs_tensors)
    )


def _bench(fn, warmup, iters):
    warmup_value = None
    for _ in range(warmup):
        warmup_value = fn()
    torch.cuda.synchronize()

    start = torch.cuda.Event(enable_timing=True)
    end = torch.cuda.Event(enable_timing=True)
    start.record()
    for _ in range(iters):
        fn()
    end.record()
    torch.cuda.synchronize()

    total_ms = start.elapsed_time(end)
    return {
        "avg_ms": total_ms / iters,
        "total_ms": total_ms,
        "warmup_value": warmup_value,
    }


def _run_case(tokens, hc, hidden, args):
    device = torch.device("cuda")
    torch.manual_seed(args.seed + tokens * 17 + hidden)
    x, residual, post_layer_mix, comb_res_mix, fn, hc_scale, hc_base, norm_weight = _make_inputs(
        tokens,
        hc,
        hidden,
        device,
    )

    core_x = _as_core(x)
    core_residual = _as_core(residual)
    core_post_layer_mix = _as_core(post_layer_mix)
    core_comb_res_mix = _as_core(comb_res_mix)
    core_fn = _as_core(fn)
    core_hc_scale = _as_core(hc_scale)
    core_hc_base = _as_core(hc_base)
    core_norm_weight = _as_core(norm_weight)

    ref_residual, ref_post, ref_comb, ref_layer_input = _make_outputs(tokens, hc, hidden, device)
    core_ref_residual = _as_core(ref_residual)
    core_ref_post = _as_core(ref_post)
    core_ref_comb = _as_core(ref_comb)
    core_ref_layer_input = _as_core(ref_layer_input)

    out_residual, out_post, out_comb, out_layer_input = _make_outputs(tokens, hc, hidden, device)
    core_out_residual = _as_core(out_residual)
    core_out_post = _as_core(out_post)
    core_out_comb = _as_core(out_comb)
    core_out_layer_input = _as_core(out_layer_input)

    ref_tensors = (ref_residual, ref_layer_input, ref_post, ref_comb)
    out_tensors = (out_residual, out_layer_input, out_post, out_comb)

    def run_ref():
        return _ref_separate(
            core_ref_residual,
            core_ref_post,
            core_ref_comb,
            core_ref_layer_input,
            core_x,
            core_residual,
            core_post_layer_mix,
            core_comb_res_mix,
            core_fn,
            core_hc_scale,
            core_hc_base,
            core_norm_weight,
            ref_tensors,
            args,
        )

    def run_op():
        _infinicore.deepseek_v4_mhc_fused_post_pre_(
            core_out_residual._underlying,
            core_out_post._underlying,
            core_out_comb._underlying,
            core_out_layer_input._underlying,
            core_x._underlying,
            core_residual._underlying,
            core_post_layer_mix._underlying,
            core_comb_res_mix._underlying,
            core_fn._underlying,
            core_hc_scale._underlying,
            core_hc_base._underlying,
            args.rms_eps,
            args.hc_pre_eps,
            args.hc_sinkhorn_eps,
            args.hc_post_mult_value,
            args.sinkhorn_repeat,
            core_norm_weight._underlying,
            args.norm_eps,
        )
        return out_tensors

    ref_perf = _bench(run_ref, args.warmup, args.iters)
    op_perf = _bench(run_op, args.warmup, args.iters)

    ref = ref_perf["warmup_value"]
    got = op_perf["warmup_value"]
    max_abs, max_rel = _tuple_max_diff(got, ref)
    allclose = _tuple_allclose(got, ref, args.atol, args.rtol)
    if not allclose:
        print(
            f"[FAIL] tokens={tokens} hc={hc} hidden={hidden} "
            f"max_abs={max_abs:.6g} max_rel={max_rel:.6g}"
        )

    return {
        "tokens": tokens,
        "ref_avg": ref_perf["avg_ms"],
        "op_avg": op_perf["avg_ms"],
        "speedup": ref_perf["avg_ms"] / op_perf["avg_ms"] if op_perf["avg_ms"] > 0 else float("inf"),
        "max_abs": max_abs,
        "max_rel": max_rel,
        "allclose": allclose,
    }


def _print_header(hc, hidden):
    print(f"\nhc={hc} hidden={hidden}")
    print(
        f"{'tokens':>8} | {'ref avg':>10} | {'op avg':>10} | "
        f"{'speedup':>8} | {'max_abs':>10} | {'max_rel':>10} | {'allclose':>8}"
    )
    print("-" * 86)


def _print_row(result):
    print(
        f"{result['tokens']:8d} | "
        f"{result['ref_avg']:10.4f} | "
        f"{result['op_avg']:10.4f} | "
        f"{result['speedup']:8.2f} | "
        f"{result['max_abs']:10.6g} | "
        f"{result['max_rel']:10.6g} | "
        f"{str(result['allclose']):>8}"
    )


def main():
    parser = argparse.ArgumentParser(description="Check and benchmark DeepSeek-V4 MHC fused post-pre.")
    parser.add_argument("--hygon", action="store_true")
    parser.add_argument("--nvidia", action="store_true")
    parser.add_argument("--tokens", default=DEFAULT_TOKENS)
    parser.add_argument("--hidden", type=int, default=4096)
    parser.add_argument("--hc", type=int, default=4)
    parser.add_argument("--warmup", type=int, default=3)
    parser.add_argument("--iters", type=int, default=40)
    parser.add_argument("--seed", type=int, default=20260812)
    parser.add_argument("--rms-eps", type=float, default=1e-6)
    parser.add_argument("--norm-eps", type=float, default=1e-6)
    parser.add_argument("--hc-pre-eps", type=float, default=1e-6)
    parser.add_argument("--hc-sinkhorn-eps", type=float, default=1e-6)
    parser.add_argument("--hc-post-mult-value", type=float, default=2.0)
    parser.add_argument(
        "--sinkhorn-repeat",
        "--sinkhorn-iters",
        dest="sinkhorn_repeat",
        type=int,
        default=20,
    )
    parser.add_argument("--atol", type=float, default=2e-2)
    parser.add_argument("--rtol", type=float, default=2e-2)
    args = parser.parse_args()

    ok = True
    _print_header(args.hc, args.hidden)
    for tokens in _parse_tokens(args.tokens):
        result = _run_case(tokens, args.hc, args.hidden, args)
        _print_row(result)
        if result["allclose"] is False:
            ok = False

    if not ok:
        raise SystemExit(1)


if __name__ == "__main__":
    main()
