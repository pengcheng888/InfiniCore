...
def _run_case(tokens, page_size, pos_dtype, out_dtype, strided, invalid_every, args):
    ...
    def ref_fn():
        return _reference_cache_kernel(kv, weight, eps, freqs, positions, out_loc, blocks, page_size, sync=False)
    ...
    _assert_cache_close(
        f"bench tokens={tokens} page_size={page_size} pos={pos_dtype} out={out_dtype} strided={strided}",
        op["warmup_value"],
        ref["warmup_value"],
        out_loc_cpu,
        page_size,
...
def _print_header(args):
    ...
    print(
        f"{'tokens':>8} | {'page':>4} | {'pos':>5} | {'out':>5} | {'strided':>7} | "
        f"{'ref avg':>10} | {'op avg':>10} | {'speedup':>7} | {'allclose':>8}"
    )
    print("-" * 88)
...
def _print_row(result):
    print(
        f"{result['tokens']:8d} | {result['page_size']:4d} | {result['pos_dtype']:>5} | "
        f"{result['out_dtype']:>5} | {str(result['strided']):>7} | "
        f"{result['ref_avg']:10.4f} | {result['op_avg']:10.4f} | "
        f"{result['speedup']:7.2f} | {str(result['allclose']):>8}"
    )
...
def main():
    ...
    for page_size in (64, 2):
        for tokens in tokens_list:
            _print_row(_run_case(tokens, page_size, torch.int32, torch.int32, False, args.invalid_every, args))
            _print_row(_run_case(tokens, page_size, torch.int64, torch.int64, False, args.invalid_every, args))
    _print_row(_run_case(19, 64, torch.int64, torch.int32, True, args.invalid_every, args))
