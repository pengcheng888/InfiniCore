...
def _reference_cache_kernel(kv, weight, eps, freqs_cis, positions, out_loc, blocks, page_size, sync=True):
...
def _run_case(tokens, pos_dtype, out_dtype, weight_dtype, strided, invalid_every, args):
    ...
    bench_out = torch.zeros_like(out)
    bench_keepalive = []
    bench_core = {
        "kv": _as_core(kv, bench_keepalive),
        "weight": _as_core(weight, bench_keepalive),
        "freqs": _as_core(freqs, bench_keepalive),
        "positions": _as_core(positions, bench_keepalive),
        "out_loc": _as_core(out_loc, bench_keepalive),
        "out": _as_core(bench_out, bench_keepalive),
    }

    def ref_fn():
        return _reference_cache_kernel(kv, weight, args.eps, freqs, positions, out_loc, blocks, args.page_size, sync=False)
    ...
    _assert_cache_close(
        f"bench tokens={tokens} pos={pos_dtype} out={out_dtype} weight={weight_dtype} strided={strided}",
        op["warmup_value"],
        _reference_cache_python(kv, weight, args.eps, freqs, positions, out_loc_cpu, blocks, args.page_size),
        out_loc_cpu,
        args.page_size,
        args.atol,
        args.rtol,
    ...
    return {
        ...
        "pos_dtype": str(positions.dtype).replace("torch.", ""),
        "out_dtype": str(out_loc.dtype).replace("torch.", ""),
...
def _print_header(args):
    print(
        f"head_dim={HEAD_DIM} rope_dim={ROPE_DIM} page_size={args.page_size} "
        f"tokens={args.tokens} iters={args.iters} warmup={args.warmup}"
    )
    print(
        f"{'tokens':>8} | {'pos':>5} | {'out':>5} | {'weight':>7} | {'strided':>7} | "
        f"{'ref avg':>10} | {'op avg':>10} | {'speedup':>7} | {'legacy':>8} | {'allclose':>8}"
    )
    print("-" * 104)
...
def _print_row(result):
    print(
        f"{result['tokens']:8d} | {result['pos_dtype']:>5} | {result['out_dtype']:>5} | "
        f"{result['weight_dtype']:>7} | {str(result['strided']):>7} | "
        f"{result['ref_avg']:10.4f} | {result['op_avg']:10.4f} | "
        f"{result['speedup']:7.2f} | {result['legacy_diff']:8d} | {str(result['allclose']):>8}"
    )
...
def main():
    ...
    for tokens in tokens_list:
        _print_row(_run_case(tokens, torch.int32, torch.int32, torch.bfloat16, False, args.invalid_every, args))
        _print_row(_run_case(tokens, torch.int64, torch.int64, torch.bfloat16, False, args.invalid_every, args))
    _print_row(_run_case(19, torch.int64, torch.int32, torch.bfloat16, True, args.invalid_every, args))
    _print_row(_run_case(23, torch.int32, torch.int64, torch.float32, False, args.invalid_every, args))
