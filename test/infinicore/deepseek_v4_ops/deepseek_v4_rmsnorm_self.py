...
def _make_input(tokens, hidden, dtype, device):
    return torch.randn((tokens, hidden), device=device, dtype=dtype).contiguous()
...
def _run_case(tokens, hidden, dtype_name, args):
    ...
    core_x = _as_core(x)
    aten_out = torch.empty_like(x)
    core_aten_out = _as_core(aten_out)
    kernel_out = torch.empty_like(x)
    core_kernel_out = _as_core(kernel_out)

    def run_aten():
        _infinicore.deepseek_v4_rmsnorm_self_aten_(core_aten_out._underlying, core_x._underlying, args.eps)
        return aten_out

    def run_kernel():
        _infinicore.deepseek_v4_rmsnorm_self_(core_kernel_out._underlying, core_x._underlying, args.eps)
        return kernel_out
...
def _print_header(dtype_name, hidden):
    print(f"\ndtype={dtype_name} hidden={hidden}")
    print(
...
def main():
    parser = argparse.ArgumentParser(description="Check and benchmark DeepSeek-V4 rmsnorm_self.")
    parser.add_argument("--hygon", action="store_true")
    parser.add_argument("--nvidia", action="store_true")
    parser.add_argument("--tokens", default=DEFAULT_TOKENS)
    parser.add_argument("--hidden", type=int, default=512)
    parser.add_argument("--dtype", choices=["bf16", "fp16", "fp32", "all"], default="all")
