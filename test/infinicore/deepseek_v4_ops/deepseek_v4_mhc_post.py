...
def _make_inputs(tokens, hc, hidden, device):
    ...
    post = (torch.rand((tokens, hc), device=device, dtype=torch.float32) * 2.0).contiguous()
    comb = torch.rand((tokens, hc, hc), device=device, dtype=torch.float32)
    comb = (comb / (comb.sum(dim=1, keepdim=True) + 1e-6)).contiguous()
    return x, residual, post, comb


def _aten_ref(core_out, core_x, core_residual, core_post, core_comb, out):
    _infinicore.deepseek_v4_mhc_post_aten_(
...
def _run_case(tokens, hc, hidden, args):
    ...
    x, residual, post, comb = _make_inputs(tokens, hc, hidden, device)

    core_x = _as_core(x)
    core_residual = _as_core(residual)
    core_post = _as_core(post)
    core_comb = _as_core(comb)

    ref_out = torch.empty((tokens, hc, hidden), device=device, dtype=torch.bfloat16)
    ...
    aten_perf = _bench(
        lambda: _aten_ref(core_ref_out, core_x, core_residual, core_post, core_comb, ref_out),
        args.warmup,
        args.iters,
    )
...
def main():
    parser = argparse.ArgumentParser(description="Check and benchmark DeepSeek-V4 MHC post.")
