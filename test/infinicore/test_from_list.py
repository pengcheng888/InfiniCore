...
def _copy_infinicore_to_torch(infinicore_tensor, torch_result_tensor):
    ...
    if tensor_to_copy.device.type != infini_device.type or (
        infini_device.type == "cuda"
        and tensor_to_copy.device.index != infini_device.index
    ):
...
def compare_with_torch(
    infinicore_tensor, expected_list, dtype=torch.float32, atol=1e-6
):
    ...
    zero_f32 = infinicore.from_list(
        [0.0] * tensor_f32.numel(), dtype=infinicore.float32
    )
...
def test_from_list_cpu_only():
    ...
    assert compare_with_torch(tensor_cpu, data, dtype=torch.float32), (
        "CPU data mismatch"
    )
    ...
    try:
        if not torch.cuda.is_available():
            ...
            torch_expected_cuda = torch.tensor(
                data, dtype=torch.float32, device="cuda:0"
            )
            torch_result_cuda = torch.zeros_like(torch_expected_cuda)
            _copy_infinicore_to_torch(tensor_cuda, torch_result_cuda)
            assert torch.allclose(torch_expected_cuda, torch_result_cuda), (
                "CUDA data mismatch"
            )
...
def test_from_list_operations():
    ...
    # Verify results
    torch_add_result = _copy_infinicore_to_torch(
        result_add, torch.zeros_like(torch_add)
    )
    assert torch.allclose(torch_add, torch_add_result), "Addition result mismatch"

    torch_mul_result = _copy_infinicore_to_torch(
        result_mul, torch.zeros_like(torch_mul)
    )
...
def test_from_list_edge_cases():
    ...
    try:
        infinicore.from_list(
            [[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=infinicore.float32
        )
