...
def load_shard_file(args):
    ...
    if not (is_fsdp_enabled() and not is_local_dist_rank_0()):
...
class PreTrainedModel(nn.Module, EmbeddingAccessMixin, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def from_pretrained(
        ...
        checkpoint_files, _ = _get_resolved_checkpoint_files_wpc(
    ...
    def _load_pretrained_model(
        ...
        model._move_missing_keys_from_meta_to_cpu(missing_keys + mismatched_keys, unexpected_keys, dtype, None)

        # correctly initialize the missing (and potentially mismatched) keys
        model._initialize_missing_keys(checkpoint_keys, ignore_mismatched_sizes, False)
        ...
        if device_map is not None and not False:
            expanded_device_map = expand_device_map(device_map, expected_keys)
            caching_allocator_warmup(model_to_load, expanded_device_map, None)
