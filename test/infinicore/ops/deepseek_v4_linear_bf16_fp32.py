...
def _run_case(m, n, k):
    ...
    x_core = _as_core(x)._underlying
    weight_core = _as_core(weight)._underlying

    out_naive = torch.empty((m, n), device="cuda", dtype=torch.float32)
    _infinicore.deepseek_v4_linear_bf16_fp32_naive_(_as_core(out_naive)._underlying, x_core, weight_core)
    infinicore.sync_stream()
    assert torch.allclose(out_naive, ref, atol=1e-4, rtol=1e-4)

    out_kernel = torch.empty((m, n), device="cuda", dtype=torch.float32)
    _infinicore.deepseek_v4_linear_bf16_fp32_kernel_(_as_core(out_kernel)._underlying, x_core, weight_core)
    infinicore.sync_stream()
    assert torch.allclose(out_kernel, out_naive, atol=2e-2, rtol=2e-2)

    out_blas = torch.empty((m, n), device="cuda", dtype=torch.float32)
    _infinicore.deepseek_v4_linear_bf16_fp32_blas_(_as_core(out_blas)._underlying, x_core, weight_core)
    infinicore.sync_stream()
    assert torch.allclose(out_blas, ref, atol=2e-2, rtol=2e-2)
