...
def _make_case(tokens, hc, hidden, seed):
    ...
    y_naive = torch.empty(tokens, hc, hidden, device=device, dtype=dtype)
...
def _run_case(tokens, hc, hidden, args):
    ...
    if args.check:
        _check_pair("mhc_post_kernel", naive, kernel, (tensors["y_naive"],), (tensors["y_kernel"],))
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA/Hygon device is required for MHC post perf test")
