...
if __name__ == "__main__":
    ...
    from infinicore.nn.modules.linear import torch_tensor_2_infini_tensor
    import torch
    ids = torch.tensor([[0,2,3]],dtype=torch.int32)
    weight = torch.randn((5, 8),dtype=torch.float32)


    ids_infini = torch_tensor_2_infini_tensor(ids, device_str="cpu")
    weight_infini = torch_tensor_2_infini_tensor(weight, device_str="cpu")

    print(weight_infini)

    inputs_embeds = infinicore.embedding(ids_infini,
                                         weight_infini)
    print(inputs_embeds)

    print("------------------>")
    import torch
    a = torch.nn.functional.embedding(ids,weight)
    print(a)
