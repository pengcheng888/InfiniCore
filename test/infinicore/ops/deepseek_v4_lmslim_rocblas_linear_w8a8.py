...
def random_w8a8_case(tokens: int, hidden: int, out_features: int, device: str):
    ...
    accum = torch.empty((tokens, out_features), device=device, dtype=torch.int32)
    return x, w, w_scale, smooth, out_ref, out_new, q_ref, q_new, scale_ref, scale_new, accum
...
def main():
    ...
    print("tokens, stable_avg_ms, rocblas_avg_ms, speedup, max_abs_diff, allclose")
    for tokens in [int(x) for x in args.tokens.split(",") if x.strip()]:
        x, w, w_scale, smooth, out_ref, out_new, q_ref, q_new, scale_ref, scale_new, accum = random_w8a8_case(
        ...
        ops.deepseek_v4_lmslim_rocblas_linear_w8a8_(
            as_tensor(out_new),
            as_tensor(x),
            weight_t_core,
            as_tensor(w_scale),
        ...
        rocblas_avg, _ = bench(
            lambda: ops.deepseek_v4_lmslim_rocblas_linear_w8a8_(
                as_tensor(out_new),
                as_tensor(x),
                weight_t_core,
                as_tensor(w_scale),
        ...
        speedup = stable_avg / rocblas_avg if rocblas_avg > 0 else float("inf")
        print(f"{tokens}, {stable_avg:.6f}, {rocblas_avg:.6f}, {speedup:.3f}, {diff:.6f}, {ok}")
        if not ok:
