...
def _unpack_qweight_int4_packed_by_rows(
    qweight_packed: torch.Tensor, in_features: int
) -> torch.Tensor:
    ...
    expanded = torch.bitwise_right_shift(
        qweight_packed[:, None, :], shifts[None, :, None]
    )
...
def _unpack_zeros_int4_packed_by_cols(
    zeros_packed: torch.Tensor, out_features: int
) -> torch.Tensor:
    ...
    expanded = torch.bitwise_right_shift(
        zeros_packed[:, :, None], shifts[None, None, :]
    )
    vals = torch.bitwise_and(expanded, 0xF).to(torch.int32)
...
def dequantize_gptq_ref(
    ...
    q = _unpack_qweight_int4_packed_by_rows(
        qweight_packed, in_features
    )  # [in_features, out_features]
    z_full = _unpack_zeros_int4_packed_by_cols(
        zeros_packed, out_features
...
def _make_packed_inputs(
    in_features: int, out_features: int, group_size: int, torch_device: str
):
    ...
    g_idx = (
        torch.arange(in_features, device=torch_device, dtype=torch.int32) // group_size
    )

    # Random nibble-level values to avoid relying on sign/shift corner cases
    q_nib = torch.randint(
        0, 16, (in_features, out_features), device=torch_device, dtype=torch.int32
    )
    z_nib = torch.randint(
        0, 16, (num_groups, out_features), device=torch_device, dtype=torch.int32
    )

    # scales in fp16
    scales = (
        torch.rand((num_groups, out_features), device=torch_device, dtype=torch.float16)
        * 0.5
        + 0.01
    )
    ...
    qweight_packed = torch.zeros(
        (in_packed, out_features), device=torch_device, dtype=torch.int32
    )
    for i in range(8):
    ...
    zeros_packed = torch.zeros(
        (num_groups, out_packed), device=torch_device, dtype=torch.int32
    )
    for j in range(8):
...
def test(
    ...
    out = TestTensor(
        (in_features, out_features), None, InfiniDtype.F16, device, mode="zeros"
    )
    ...
    if PROFILE:
        profile_operation(
            "PyTorch",
            lambda: dequantize_gptq_ref(
                qweight_packed, scales, zeros_packed, g_idx, group_size
            ),
            device,
            NUM_PRERUN,
            NUM_ITERATIONS,
        ...
        profile_operation(
            "    lib", lambda: lib_dequantize_gptq(), device, NUM_PRERUN, NUM_ITERATIONS
        )
