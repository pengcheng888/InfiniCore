...
_TEST_CASES = [
    # Basic cases
    ((1, 64, 112, 112), 3, 3, 2, 2, 1, 1, 1, 1, 0),
]
...
def max_pool2d_ref(input, kernel_size_h, kernel_size_w, stride_h, stride_w, 
                   padding_h, padding_w, dilation_h, dilation_w, ceil_mode):
    """
    Reference implementation of max_pool2d with dilation support.
    For dilation=1, uses PyTorch's built-in max_pool2d.
    For dilation>1, implements dilated max pooling manually.
    """
    if dilation_h == 1 and dilation_w == 1:
        ...
        # Pad input (use negative infinity for max pooling)
        if padding_h > 0 or padding_w > 0:
            input_padded = F.pad(input, (padding_w, padding_w, padding_h, padding_h), mode='constant', value=float('-inf'))
        else:
            input_padded = input
        
        # Initialize output with negative infinity
        output = torch.full((N, C, out_h, out_w), float('-inf'), dtype=input.dtype, device=input.device)
        
        # Perform dilated max pooling
        for i in range(out_h):
            for j in range(out_w):
                ...
                max_val = torch.full((N, C), float('-inf'), dtype=input.dtype, device=input.device)
                for kh in range(kernel_size_h):
                    for kw in range(kernel_size_w):
                        ...
                        if h_idx < input_padded.shape[2] and w_idx < input_padded.shape[3]:
                            max_val = torch.max(max_val, input_padded[:, :, h_idx, w_idx])
...
def infer_output_shape(input_shape, kernel_size_h, kernel_size_w, stride_h, stride_w,
                       padding_h, padding_w, dilation_h, dilation_w, ceil_mode):
    """Infer output shape for max_pool2d"""
    N, C, H, W = input_shape
    eff_kernel_h = (kernel_size_h - 1) * dilation_h + 1
    eff_kernel_w = (kernel_size_w - 1) * dilation_w + 1
...
def test(
    ...
    print(
        f"Testing MaxPool2d on {InfiniDeviceNames[device]} with "
    ...
    check_error(
        LIBINFINIOP.infiniopCreateMaxPool2dDescriptor(
            handle,
    ...
    check_error(
        LIBINFINIOP.infiniopGetMaxPool2dWorkspaceSize(
    ...
    if PROFILE:
        ...
        profile_operation("    lib", lib_max_pool2d, device, NUM_PRERUN, NUM_ITERATIONS)
        # fmt: on
    
    check_error(LIBINFINIOP.infiniopDestroyMaxPool2dDescriptor(descriptor))
