...
class RoPE(Module):
    def __init__(
        ...
        global _cos_table
        if _sin_table is None:
            _sin_table, _cos_table = create_sin_cos_table(
                self.max_position_embeddings,
                head_dim=self.head_dim,
                theta=self.rope_theta,
                **factory_kwargs,
    ...
    def forward(self, states: Tensor, position_ids: Tensor, algo=RopeAlgo.GPT_NEOX):
        ...
        states_out = infinicore.empty_like(states)
        for ib in range(0, bs):
            state_in = infinicore.narrow(states, 0, ib, 1).view(
                (1 * seq_len, num_attention_heads, head_dim)
            )
            state_out = infinicore.narrow(states_out, 0, ib, 1).view(
                (1 * seq_len, num_attention_heads, head_dim)
            )

            F.rope(
                state_in, position_ids, _sin_table, _cos_table, algo=algo, out=state_out
            )
