...
INFINIOP_MOORE_KERNEL flashAttentionDecodeHd576Warp(
    Tdata *out,
    const Tdata *q,
    const Tdata *k_cache,
    const Tdata *v_cache,
    ...
    ptrdiff_t o_stride) {
    op::paged_attention::cuda::flashAttentionDecodeWarpKernel<Tindex, Tdata, 576>(
...
template <typename Tindex, typename Tdata>
INFINIOP_MOORE_KERNEL flashAttentionDecodeHd576SplitKv(
    float *partial_acc,
    float *partial_m,
    float *partial_l,
    const Tdata *q,
    ...
    ptrdiff_t v_head_stride,
    int num_splits) {
    op::paged_attention::cuda::flashAttentionDecodeSplitKvWarpKernel<Tindex, Tdata, 576>(
...
INFINIOP_MOORE_KERNEL flashAttentionDecodeHd576SplitKvCombine(
    Tdata *out,
    ...
    int num_splits,
    ptrdiff_t o_stride) {
    op::paged_attention::cuda::flashAttentionDecodeSplitKvCombineWarpKernel<Tdata, 576>(
        out, partial_acc, partial_m, partial_l, num_splits, o_stride);
}

template <typename Tindex>
infiniStatus_t launch_decode_hd576_impl(
    void *workspace,
    ...
    if (use_split) {
        ...
        const size_t acc_elems = static_cast<size_t>(num_splits) * n * 576;
        ...
        if (dtype == INFINI_DTYPE_F16) {
            flashAttentionDecodeHd576SplitKv<Tindex, half><<<grid_split, block, 0, stream>>>(
                partial_acc, partial_m, partial_l,
            ...
            flashAttentionDecodeHd576SplitKvCombine<half><<<grid, block, 0, stream>>>(
                static_cast<half *>(out), partial_acc, partial_m, partial_l, num_splits, o_stride);
            return INFINI_STATUS_SUCCESS;
        }
        if (dtype == INFINI_DTYPE_BF16) {
            flashAttentionDecodeHd576SplitKv<Tindex, __nv_bfloat16><<<grid_split, block, 0, stream>>>(
                partial_acc, partial_m, partial_l,
                ...
                q_stride, k_batch_stride, k_row_stride, k_head_stride,
                v_batch_stride, v_row_stride, v_head_stride, num_splits);
            flashAttentionDecodeHd576SplitKvCombine<__nv_bfloat16><<<grid, block, 0, stream>>>(
    ...
    if (dtype == INFINI_DTYPE_F16) {
        flashAttentionDecodeHd576Warp<Tindex, half><<<grid, block, 0, stream>>>(
    ...
    if (dtype == INFINI_DTYPE_BF16) {
        flashAttentionDecodeHd576Warp<Tindex, __nv_bfloat16><<<grid, block, 0, stream>>>(
...
infiniStatus_t launch_decode_hd576_i64(
    ...
    musaStream_t stream) {
    return launch_decode_hd576_impl<int64_t>(
...
infiniStatus_t launch_decode_hd576_i32(
    ...
    ptrdiff_t v_batch_stride, ptrdiff_t v_row_stride, ptrdiff_t v_head_stride, ptrdiff_t o_stride,
    musaStream_t stream) {
    return launch_decode_hd576_impl<int32_t>(
        workspace, workspace_size, out, q, k_cache, v_cache, dtype, block_tables, cache_lens, alibi_slopes,
        num_heads, num_seqs, num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        q_stride, k_batch_stride, k_row_stride, k_head_stride,
        v_batch_stride, v_row_stride, v_head_stride, o_stride, stream);
...
infiniStatus_t launch_decode_hd576_u32(
    ...
    ptrdiff_t v_batch_stride, ptrdiff_t v_row_stride, ptrdiff_t v_head_stride, ptrdiff_t o_stride,
    musaStream_t stream) {
    return launch_decode_hd576_impl<uint32_t>(
        workspace, workspace_size, out, q, k_cache, v_cache, dtype, block_tables, cache_lens, alibi_slopes,
        num_heads, num_seqs, num_kv_heads, scale, max_num_blocks_per_seq, page_block_size,
        q_stride, k_batch_stride, k_row_stride, k_head_stride,
        v_batch_stride, v_row_stride, v_head_stride, o_stride, stream);
