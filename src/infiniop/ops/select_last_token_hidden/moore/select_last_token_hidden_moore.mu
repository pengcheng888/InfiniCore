...
#include "select_last_token_hidden_moore.h"

#include <cstdint>
#include <musa_runtime.h>

template <typename CopyT>
INFINIOP_MOORE_KERNEL selectLastTokenHiddenKernel(
    CopyT *__restrict__ output,
    const CopyT *__restrict__ hidden_states,
    const int32_t *__restrict__ input_offsets,
    size_t row_width,
    size_t total_tokens) {
    op::select_last_token_hidden::cuda::selectLastTokenHiddenBlock(
        output,
        hidden_states,
        input_offsets,
        row_width,
...
infiniStatus_t Descriptor::calculate(
    ...
    const auto musa_stream = reinterpret_cast<musaStream_t>(stream);
    if (_row_bytes % sizeof(uint4) == 0) {
        selectLastTokenHiddenKernel<uint4><<<_num_requests, block_size, 0, musa_stream>>>(
        ...
        selectLastTokenHiddenKernel<uint8_t><<<_num_requests, block_size, 0, musa_stream>>>(
