...
__C infiniStatus_t infiniopCreateAvgPool2dDescriptor(
...
#define CREATE(CASE, NAMESPACE)                                                   \
    case CASE:                                                                    \
        return op::avg_pool2d::NAMESPACE::Descriptor::create(                     \
            handle,                                                               \
            reinterpret_cast<op::avg_pool2d::NAMESPACE::Descriptor **>(desc_ptr), \
            output_desc,                                                          \
            input_desc,                                                           \
            kernel_size_h,                                                        \
            kernel_size_w,                                                        \
...
__C infiniStatus_t infiniopGetAvgPool2dWorkspaceSize(
    infiniopAvgPool2dDescriptor_t desc,
    size_t *size) {

#define GET_SIZE(CASE, NAMESPACE)                                                     \
    case CASE:                                                                        \
        *size = reinterpret_cast<const op::avg_pool2d::NAMESPACE::Descriptor *>(desc) \
                    ->get_workspace_size();                                           \
...
__C infiniStatus_t infiniopAvgPool2d(
    infiniopAvgPool2dDescriptor_t desc,
    void *workspace,
...
#define CALCULATE(CASE, NAMESPACE)                                                   \
    case CASE:                                                                       \
        return reinterpret_cast<const op::avg_pool2d::NAMESPACE::Descriptor *>(desc) \
...
__C infiniStatus_t infiniopDestroyAvgPool2dDescriptor(
    infiniopAvgPool2dDescriptor_t desc) {

#define DESTROY(CASE, NAMESPACE)                                                \
    case CASE:                                                                  \
        delete reinterpret_cast<op::avg_pool2d::NAMESPACE::Descriptor *>(desc); \
        return INFINI_STATUS_SUCCESS;
