...
public:
    ...
    static utils::Result<BroadcastToInfo> create(
        ...
        if (input_descs.size() != 1) return INFINI_STATUS_BAD_PARAM;
        auto input_desc = input_descs[0];

        if (out_desc->dtype() != input_desc->dtype()) return INFINI_STATUS_BAD_TENSOR_DTYPE;
        if (out_desc->ndim() < input_desc->ndim()) return INFINI_STATUS_BAD_TENSOR_SHAPE;
        if (out_desc->ndim() > MAX_DIM) return INFINI_STATUS_BAD_PARAM;

        BroadcastToInfo info(out_desc->dtype(), out_desc->ndim(), 0);
        ...
        int offset = info._ndim - input_desc->ndim();
