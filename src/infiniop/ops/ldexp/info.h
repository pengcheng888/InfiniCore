...
public:
    ...
    static utils::Result<LdexpInfo> create(
        ...
        int ndim_y = y_desc->ndim();
        int ndim_x = x_desc->ndim();
        int ndim_exp = exp_desc->ndim();
        ...
        auto compute_strides = [&](infiniopTensorDescriptor_t input_desc, std::vector<int>& out_strides) {
            int input_ndim = input_desc->ndim();
            ...
            for (int i = input_ndim - 1; i >= 0; --i) {
                ...
                current_stride *= input_desc->shape()[i];
            }
            ...
            for (int i = 0; i < ndim_out; ++i) {
                if (i < offset) {
                    ...
                    int input_dim_size = input_desc->shape()[input_dim_idx];
