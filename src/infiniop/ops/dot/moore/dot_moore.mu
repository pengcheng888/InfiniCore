...
infiniStatus_t Descriptor::calculate(
    ...
    case INFINI_DTYPE_F16: {
        ...
        CHECK_MOORE(cudaMallocAsync(&result_f, sizeof(float), cuda_stream));
        CHECK_MOORE(cudaMemsetAsync(result_f, 0, sizeof(float), cuda_stream));
        ...
        float result_val;
        CHECK_MOORE(cudaMemcpyAsync(&result_val, result_f, sizeof(float), cudaMemcpyDeviceToHost, cuda_stream));
        CHECK_MOORE(cudaStreamSynchronize(cuda_stream));
        *reinterpret_cast<half *>(y) = __float2half(result_val);
        CHECK_MOORE(cudaFreeAsync(result_f, cuda_stream));
        break;
    }
    ...
    case INFINI_DTYPE_F64: {
        ...
        CHECK_MOORE(cudaMemsetAsync(result_d, 0, sizeof(double), cuda_stream));
