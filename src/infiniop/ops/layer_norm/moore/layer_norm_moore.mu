...
INFINIOP_MOORE_KERNEL launchKernel(
    Tdata *output,
...
INFINIOP_MOORE_KERNEL blockLayernorm(
    Tdata *output,
    const Tdata *input,
    const Tdata *weight,
    const Tdata *bias,
...
INFINIOP_MOORE_KERNEL warpLayernorm(
    Tdata *output,
    const Tdata *input,
    const Tdata *weight,
    const Tdata *bias,
...
infiniStatus_t calculate_layer_norm(
    ...
    CHECK_MOORE(musaMemcpyAsync(input_strides_cuda, info.input_strides.data(), sizeof(ptrdiff_t) * ndim, musaMemcpyHostToDevice, stream));
    CHECK_MOORE(musaMemcpyAsync(output_strides_cuda, info.output_strides.data(), sizeof(ptrdiff_t) * ndim, musaMemcpyHostToDevice, stream));
    CHECK_MOORE(musaMemcpyAsync(input_standardization_strides_cuda, info.input_standardization_strides.data(), sizeof(ptrdiff_t) * (ndim - 1), musaMemcpyHostToDevice, stream));
    CHECK_MOORE(musaMemcpyAsync(input_std_deviation_strides_cuda, info.input_std_deviation_strides.data(), sizeof(ptrdiff_t) * (ndim - 1), musaMemcpyHostToDevice, stream));
    CHECK_MOORE(musaMemcpyAsync(shape_cuda, info.input_shape.data(), sizeof(size_t) * ndim, musaMemcpyHostToDevice, stream));
...
#define CALCULATE_LAYER_NORM(BLOCK_SIZE, TDATA)                             \
    calculate_layer_norm<BLOCK_SIZE, TDATA>(_info,                          \
                                            (TDATA *)output,                \
                                            (TDATA *)input_standardization, \
                                            (TDATA *)input_std_deviation,   \
                                            (const TDATA *)input,           \
                                            (const TDATA *)weight,          \
                                            (const TDATA *)bias, stream, workspace)

#define CALCULATE_LAYER_NORM_WITH_MOORE_BLOCK(BLOCK_SIZE)           \
    {                                                               \
    ...
    if (_opaque->internal->maxThreadsPerBlock() == MOORE_BLOCK_SIZE_1024) {
        CALCULATE_LAYER_NORM_WITH_MOORE_BLOCK(MOORE_BLOCK_SIZE_1024)
    } else if (_opaque->internal->maxThreadsPerBlock() == MOORE_BLOCK_SIZE_512) {
        CALCULATE_LAYER_NORM_WITH_MOORE_BLOCK(MOORE_BLOCK_SIZE_512)
    } else {
        return INFINI_STATUS_DEVICE_ARCHITECTURE_NOT_SUPPORTED;
    }
    return INFINI_STATUS_SUCCESS;

#undef CALCULATE_LAYER_NORM_WITH_MOORE_BLOCK
