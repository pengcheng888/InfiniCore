...
__global__ void Marlin(
    ...
    if constexpr (a_type_id == vllm::kFE4M3fn.id()) {
        return;
    }
#endif
    ...
    // num_bits == 4
    constexpr bool use_fp16_accum = a_type_id == vllm::kFloat16.id() && (!(b_type_id == vllm::kFE2M1f.id() && s_type_id == vllm::kFE4M3fn.id()) && !(group_blocks == -1 && num_bits == 4));
#else
    ...
    if constexpr (b_type == vllm::kFE2M1f) {
        static_assert(s_type == vllm::kFE4M3fn && group_blocks == 1 || s_type == vllm::kFE8M0fnu && group_blocks == 2);
    ...
    constexpr bool is_int_type = b_type == vllm::kU4 || b_type == vllm::kU8 || b_type == vllm::kS4 || b_type == vllm::kS8 || b_type == vllm::kU4B8 || b_type == vllm::kU8B128;
    // see comments of dequant.h for more details
    constexpr bool dequant_skip_flop = is_a_8bit || b_type == vllm::kFE4M3fn || b_type == vllm::kFE2M1f && s_type == vllm::kFE4M3fn || has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value || has_zp && !is_zp_float && !(b_type == vllm::kU8);
    ...
    auto init_part2_slice = [&]() {
        ...
        if (first_init && use_atomic_add && slice_count > 1 && slice_idx == 0) {
            ...
            if (m_block_size_8) {
                m_per_thread = div_ceil(8, threads / threads_per_m);
            }
    ...
    constexpr int s_sh_stride = 16 * thread_n_blocks / (b_type == vllm::kFE2M1f ? 16 : 8);
    ...
    // Shared read index.
    int a_sh_rd = a_sh_stride * ((threadIdx.x % 32) % (16 / (m_block_size_8 ? 2 : 1))) + (threadIdx.x % 32) / (16 / (m_block_size_8 ? 2 : 1));
    ...
    if constexpr (!has_act_order) {
        if constexpr (group_blocks == -1) {
            ...
            s_gl_rd = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks + threadIdx.x / s_sh_stride) + s_sh_stride * slice_col + threadIdx.x % s_sh_stride;
        }
    }
    ...
    if constexpr (has_zp) {
        if constexpr (group_blocks == -1) {
            ...
            zp_gl_rd = zp_gl_stride * ((thread_k_blocks * slice_row) / group_blocks + threadIdx.x / zp_sh_stride) + zp_sh_stride * slice_col + threadIdx.x % zp_sh_stride;
        }
    }
    ...
    if constexpr (has_zp) {
        if constexpr (is_zp_float) {
            ...
            zp_sh_rd = num_ints_per_thread * num_col_threads * ((threadIdx.x / 32) % tb_n_warps / 2) + num_ints_per_thread * ((threadIdx.x % 32) / num_row_threads);
        } else {
            zp_sh_rd = num_ints_per_thread * num_col_threads * ((threadIdx.x / 32) % tb_n_warps) + num_ints_per_thread * ((threadIdx.x % 32) / num_row_threads);
        }
    }
...
#pragma unroll
    ...
    auto fetch_act_order_scales_to_shared = [&](bool is_async, int first_group_id,
        ...
        if (is_async) {
            ...
            for (int i = 0; i < sh_num_groups; i++) {
                if (threadIdx.x < s_sh_stride) {
                    sh_s[(i * s_sh_stride) + threadIdx.x] = scales_ptr[row_offset + (i * s_gl_stride) + slice_n_offset + threadIdx.x];
                }
            }
        }
    };
...
#pragma unroll
    ...
    auto fetch_scales_to_registers = [&](int k, int full_pipe) {
        ...
        if (is_same_group[pipe]) {
            if (k % 2 == 0) {
                *(reinterpret_cast<int4 *>(&(act_frag_s[k % 2][0][0]))) = sh_s[(same_group_id[pipe] - sh_first_group_id) * s_sh_stride + s_col_shift];
            } else {
                *(reinterpret_cast<int4 *>(&(act_frag_s[k % 2][0][0]))) = *(reinterpret_cast<int4 *>(&(act_frag_s[(k - 1) % 2][0][0])));
            }
...
#pragma unroll
    ...
    auto matmul = [&](int k, int pipe) {
        ...
        const bool is_new_zp = (group_blocks == 0) || ((group_blocks > 0) && (group_blocks < b_sh_wr_iters || k == 0)) && (pipe % g == 0) || (group_blocks == -1 && is_first_matmul_in_slice);
        if constexpr (has_zp && !is_zp_float) {
            if (is_new_zp) {
                if constexpr (group_blocks == -1) {
...
#pragma unroll
        for (int j = 0; j < 4; j++) {
            ...
            if constexpr (has_act_order && !is_a_8bit) {
                ...
                if (is_new_zp)
                    frag_zp[j] = __hmul2(frag_zp[j], s2);
...
#pragma unroll
                            for (int g = 0; g < 4; g++) {
                                ...
                                *reinterpret_cast<int32_t *>(&frag_c[i][j][0][g]) += *reinterpret_cast<int32_t *>(&frag_c_tmp[i][j][0][g]) * scale;
                                frag_c_tmp[i][j][0][g] = 0.0f;
                            }
...
#pragma unroll
                            for (int g = 0; g < 4; g++) {
                                ...
                                *reinterpret_cast<int32_t *>(&frag_c[i][j][1][g]) += *reinterpret_cast<int32_t *>(&frag_c_tmp[i][j][1][g]) * scale;
                                frag_c_tmp[i][j][1][g] = 0.0f;
                            }
                        }
                    } else {
...
#pragma unroll
                        for (int j = 0; j < 4; j++)
                            reinterpret_cast<FragC *>(
                                frag_c)[(is_a_8bit ? 2 : 4) * 2 * m_block + i][j]
                                += c_rd[j];
                    }
                }
                __syncthreads();
    ...
    auto global_reduce_fp16 = [&](bool first = false, bool last = false) {
        ...
        if (threadIdx.x < active_threads) {
            ...
            if constexpr (m_block_size_8) {
                ...
                c_gl_wr = c_gl_stride * ((threadIdx.x % 32) / 4) * (is_a_8bit ? 2 : 1) + 4 * (threadIdx.x / 32) + threadIdx.x % 4;
...
#pragma unroll
            for (int i = 0; i < (m_block_size_8 ? 2 : thread_m_blocks * 4); i++) {
                bool mask = (!m_block_size_8) && (i < (thread_m_blocks - 1) * 4 || 8 * (i / 2) + row < prob_m) || (m_block_size_8) && ((threadIdx.x % 4) * 2 + i < prob_m);
                if (mask) {
                    if (!first) {
...
#pragma unroll
    ...
    auto write_result = [&](bool last) {
        ...
        auto write = [&](int idx, float c0, float c1, FragS &s, FragS &b_bias) {
            ...
            // 4-bit)
            if constexpr (!has_act_order && group_blocks == -1 && !is_a_8bit && b_type.size_bits() == 4 && (has_zp && dequant_skip_flop || !has_zp)) {
...
#pragma unroll
            ...
            } else if (!has_act_order && group_blocks == -1 && b_type.size_bits() == 8 && (has_zp && dequant_skip_flop || !has_zp)) {
                if (threadIdx.x / 32 < tb_n_warps) {
#pragma unroll
                    for (int i = 0; i < thread_m_blocks; i++) {
#pragma unroll
