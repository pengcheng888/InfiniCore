...
[[noreturn]] inline auto panic(DebugInfo location, Args &&...args) -> void {
...
/// Python-style integer range: irange(n) -> [0, n)
template <typename T>
IntegerRange<T> irange(T end) {
    return {0, end};
}

/// Python-style integer range: irange(start, end) -> [start, end)
template <typename T>
