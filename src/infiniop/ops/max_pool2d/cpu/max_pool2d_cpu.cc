...
template <typename T>
infiniStatus_t max_pool2d_cpu_impl(
    ...
    T *output,
    const T *input) {
    // Get minimum value for the data type (for padding)
    T min_val;
    if constexpr (std::is_same<T, fp16_t>::value || std::is_same<T, bf16_t>::value) {
        min_val = utils::cast<T>(std::numeric_limits<float>::lowest());
    } else {
                    ...
                    for (int kh = 0; kh < kernel_size_h; ++kh) {
                        for (int kw = 0; kw < kernel_size_w; ++kw) {
                            ...
                            if (h_idx >= 0 && h_idx < static_cast<int>(H) && w_idx >= 0 && w_idx < static_cast<int>(W)) {
                                ...
                                if constexpr (std::is_same<T, fp16_t>::value || std::is_same<T, bf16_t>::value) {
                                    float val_f = utils::cast<float>(val);
                                    float max_f = utils::cast<float>(max_val);
                                    if (val_f > max_f) {
                                        max_val = val;
                                    }
                                } else {
                            ...
                            // If out of bounds, padding value (min_val) is already used
                        }
                    }

                    output_base[oh * output_stride_h + ow * output_stride_w] = max_val;
                }
            }
        }
    }
...
infiniStatus_t Descriptor::calculate(
    ...
    case INFINI_DTYPE_F16:
        return max_pool2d_cpu_impl<fp16_t>(
            N, C, H, W, out_h, out_w,
    ...
    case INFINI_DTYPE_F32:
        return max_pool2d_cpu_impl<float>(
            N, C, H, W, out_h, out_w,
            _input_strides[0], _input_strides[1], _input_strides[2], _input_strides[3],
            _output_strides[0], _output_strides[1], _output_strides[2], _output_strides[3],
            _kernel_size_h, _kernel_size_w,
    ...
    case INFINI_DTYPE_BF16:
        return max_pool2d_cpu_impl<bf16_t>(
            N, C, H, W, out_h, out_w,
            _input_strides[0], _input_strides[1], _input_strides[2], _input_strides[3],
            _output_strides[0], _output_strides[1], _output_strides[2], _output_strides[3],
            _kernel_size_h, _kernel_size_w,
