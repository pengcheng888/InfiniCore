...
void topkrouter_cpu_one_token(float *values_input,
                              int *indices_input,
                              const float *x_input, // 先假定是 float
                              const float *correction_bias,
                              const float routed_scaling_factor,
                              const size_t topk,
                              const size_t width,
                              const size_t n_routed_experts,
    ...
    for (size_t i = 0; i < width; ++i) {
        value_index_arr[i].first = sigmoid_func(x_input[i]);
...
infiniStatus_t topkrouter_cpu_func(float *values,
                                   int *indices,
    ...
    */
    for (size_t n = 0; n < N; ++n) {
        ...
        topkrouter_cpu_one_token(values_input, indices_input, x_input,
...
infiniStatus_t Descriptor::calculate(
    ...
    if (256 == width) {
        topkrouter_cpu_func(values, indices, static_cast<const float *>(x), correction_bias, routed_scaling_factor,
                            topk,
                            N, width,
                            n_routed_experts,
                            n_group,
