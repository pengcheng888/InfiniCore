...
__global__ void Marlin(
    ...
    constexpr bool dequant_skip_flop = !is_int_type || has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value || has_zp && !is_zp_float && !(w_type == host::kU8);
    ...
    if constexpr (!has_act_order) {
        if constexpr (group_blocks == -1) {
            ...
            s_gl_rd = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) / (w_type == host::kFE2M1f ? 2 : 1) + s_sh_stride * slice_col + threadIdx.x;
        }
    }
    ...
    if constexpr (has_zp) {
        if constexpr (is_zp_float) {
            ...
            zp_sh_rd = num_ints_per_thread * num_col_threads * ((threadIdx.x / 32) % (thread_n_blocks / 4)) + num_ints_per_thread * ((threadIdx.x % 32) / num_row_threads);
        }
    }
            ...
            if constexpr (group_blocks == -1) {
                ...
                if constexpr (group_blocks >= thread_k_blocks) {
                    ...
                    if constexpr (w_type_id != host::kFE2M1f.id()) {
                        ...
                        reinterpret_cast<int2 *>(&frag_s[k % 2])[0] = reinterpret_cast<int2 *>(sh_s_stage)[s_sh_rd + cur_group_id * (2 * s_sh_stride)];
                    }
                }
            }
...
#pragma unroll
                ...
                if (k % b_sh_wr_iters == 0) {
                    int4 *sh_zp_stage = sh_zp + zp_sh_stage * ((group_blocks / thread_k_blocks) * (pipe / (group_blocks / thread_k_blocks)));
#pragma unroll
                    for (int i = 0; i < num_ints_per_thread; i++) {
...
#pragma nv_diagnostic pop
    ...
    auto matmul = [&](int k) {
        int k2 = k % 2;
        const bool is_new_zp = ((group_blocks != -1) && (group_blocks < thread_k_blocks || k == 0)) || (group_blocks == -1 && is_first_matmul_in_slice);
        if constexpr (has_zp && !is_zp_float) {
            if (is_new_zp) {
                if constexpr (group_blocks == -1) {
...
#pragma unroll
        for (int j = 0; j < 4; j++) {
            ...
            if constexpr (has_act_order) {
                ...
                if (is_new_zp)
                    frag_zp[j] = __hmul2(frag_zp[j], *reinterpret_cast<scalar_t2 *>(&frag_s[k2][j]));
...
#pragma unroll
                        for (int j = 0; j < 2 * 4; j++) {
                            ...
                            reinterpret_cast<float *>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4) + delta] += Dtype::num2float(reinterpret_cast<scalar_t *>(&c_red)[j]);
                        }
                    }
                    if (!last) {
                        int4 c;
#pragma unroll
                        for (int j = 0; j < 2 * 4; j++) {
                            ...
                            reinterpret_cast<scalar_t *>(&c)[j] = Dtype::float2num(reinterpret_cast<float *>(&frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4) + delta]);
                        }
                        if constexpr (m_block_size_8)
