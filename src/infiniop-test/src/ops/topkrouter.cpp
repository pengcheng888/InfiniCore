...
std::shared_ptr<Test> Test::build(
    ...
    if (attributes.find("routed_scaling_factor") == attributes.end()
        ...
        || tensors.find("values") == tensors.end()
        || tensors.find("indices") == tensors.end()
        || tensors.find("x") == tensors.end()
        || tensors.find("correction_bias") == tensors.end()
        || tensors.find("lable_values") == tensors.end()
...
std::shared_ptr<infiniop_test::Result> Test::run(
    ...
    CHECK_OR(infiniopCreateTopkrouterDescriptor(handle, &op_desc,
                                                _attributes->x->desc(),
    ...
    CHECK_OR(infiniopTopkrouter(op_desc,
