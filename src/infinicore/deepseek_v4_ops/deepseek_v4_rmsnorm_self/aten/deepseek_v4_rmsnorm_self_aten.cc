...
void check_shapes(const Tensor &out, const Tensor &x) {
    if (x->ndim() < 1) {
        throw std::runtime_error("deepseek_v4_rmsnorm_self_aten_ expects rank >= 1.");
    }
    if (out->shape() != x->shape()) {
        throw std::runtime_error("deepseek_v4_rmsnorm_self_aten_ output shape mismatch.");
    }
    if (out->dtype() != x->dtype()) {
        throw std::runtime_error("deepseek_v4_rmsnorm_self_aten_ output dtype mismatch.");
    }
}
...
Tensor deepseek_v4_rmsnorm_self_aten(const Tensor &x, float epsilon) {
    ...
    deepseek_v4_rmsnorm_self_aten_(out, x, epsilon);
    return out;
}
...
void deepseek_v4_rmsnorm_self_aten_(Tensor out, const Tensor &x, float epsilon) {
...
#endif
    check_device_and_guard(out, x, "deepseek_v4_rmsnorm_self_aten_");
    check_shapes(out, x);
    ...
    (void)x;
    (void)epsilon;
    throw std::runtime_error("deepseek_v4_rmsnorm_self_aten_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
