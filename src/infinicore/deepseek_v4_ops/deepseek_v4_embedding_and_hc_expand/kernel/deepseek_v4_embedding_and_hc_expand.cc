...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(DeepseekV4EmbeddingAndHcExpand);
...
DeepseekV4EmbeddingAndHcExpand::DeepseekV4EmbeddingAndHcExpand(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    INFINICORE_ASSERT_TENSORS_SAME_DEVICE(out, input, weight);
    INFINICORE_GRAPH_OP_DISPATCH(out->device().getType(), out, input, weight, hc_mult);
}

void DeepseekV4EmbeddingAndHcExpand::execute(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(DeepseekV4EmbeddingAndHcExpand, out, input, weight, hc_mult);
}

namespace deepseek_v4_embedding_and_hc_expand_impl {
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(DeepseekV4EmbeddingAndHcExpand,
                                       &deepseek_v4_embedding_and_hc_expand_impl::plan,
                                       &deepseek_v4_embedding_and_hc_expand_impl::run,
                                       &deepseek_v4_embedding_and_hc_expand_impl::cleanup);
} // namespace deepseek_v4_embedding_and_hc_expand_kernel_register
...
#if defined(ENABLE_HYGON_API) || defined(ENABLE_NVIDIA_API)
    check_kernel_tensors(out, input, weight, hc_mult, "deepseek_v4_embedding_and_hc_expand_kernel_");
    deepseek_v4::DeepseekV4EmbeddingAndHcExpand::execute(out, input, weight, hc_mult);
#else
    (void)out;
    (void)input;
    (void)weight;
