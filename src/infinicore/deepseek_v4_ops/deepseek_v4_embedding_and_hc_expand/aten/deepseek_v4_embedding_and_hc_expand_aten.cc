...
Tensor deepseek_v4_embedding_and_hc_expand_aten(const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    auto out = Tensor::empty(output_shape_for_aten(input, weight, hc_mult, "deepseek_v4_embedding_and_hc_expand_aten"), weight->dtype(), weight->device());
    deepseek_v4_embedding_and_hc_expand_aten_(out, input, weight, hc_mult);
    return out;
}
...
void deepseek_v4_embedding_and_hc_expand_aten_(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    ...
    check_accelerator_tensor_aten(out, "deepseek_v4_embedding_and_hc_expand_aten_");
#if defined(ENABLE_HYGON_API)
    ...
    check_same_device_aten(out, input, weight, "deepseek_v4_embedding_and_hc_expand_aten_");
    check_shapes_and_dtypes_aten(out, input, weight, hc_mult, "deepseek_v4_embedding_and_hc_expand_aten_");
    ...
    (void)weight;
    (void)hc_mult;
    throw std::runtime_error("deepseek_v4_embedding_and_hc_expand_aten_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
