...
__device__ __forceinline__ void store_fp8_pair(uint8_t *__restrict__ dst, int pair_idx, float x, float y) {
    const uint16_t packed = static_cast<uint16_t>(fp8_e4m3_byte(x)) | (static_cast<uint16_t>(fp8_e4m3_byte(y)) << 8);
...
__global__ void compress_norm_rope_store_kernel(const void *__restrict__ kv,
    ...
    if constexpr (PageBits >= 0) {
        ...
        constexpr int64_t kStaticPageBytes = ((kStaticBytes + kValueBytesPerToken - 1) / kValueBytesPerToken) * kValueBytesPerToken;
    ...
    if (warp_id == kRopeWarp) {
        ...
        if (lane == 0) {
            ...
            if constexpr (PageBits >= 0) {
                scale_ptr = page_ptr + (static_cast<int64_t>(kValueBytesPerToken) << PageBits) + offset * kScaleBytesPerToken;
            } else {
                scale_ptr = page_ptr + static_cast<int64_t>(kValueBytesPerToken) * page_size + offset * kScaleBytesPerToken;
            }
