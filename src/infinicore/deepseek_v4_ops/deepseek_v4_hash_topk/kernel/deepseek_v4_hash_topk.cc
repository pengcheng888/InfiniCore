...
void check_shapes(const Tensor &topk_weights,
    ...
    if (router_logits->ndim() != 2) {
        throw std::runtime_error("deepseek_v4_hash_topk_ expects router_logits to be 2-D.");
    }
    if (input_ids->ndim() != 1) {
        throw std::runtime_error("deepseek_v4_hash_topk_ expects input_ids to be 1-D.");
    }
    if (tid2eid->ndim() != 2) {
        throw std::runtime_error("deepseek_v4_hash_topk_ expects tid2eid to be 2-D.");
    }
    if (topk_weights->shape() != topk_indices->shape()) {
        throw std::runtime_error("deepseek_v4_hash_topk_ topk weight/index shape mismatch.");
    }
    if (num_fused_shared_experts < 0) {
        throw std::runtime_error("deepseek_v4_hash_topk_ expects num_fused_shared_experts >= 0.");
    }
    if (topk_weights->shape() != Shape{router_logits->size(0), tid2eid->size(1) + num_fused_shared_experts}) {
        throw std::runtime_error("deepseek_v4_hash_topk_ output shape mismatch.");
    }
    if (input_ids->size(0) != router_logits->size(0)) {
        throw std::runtime_error("deepseek_v4_hash_topk_ input_ids/router token count mismatch.");
    }
}
