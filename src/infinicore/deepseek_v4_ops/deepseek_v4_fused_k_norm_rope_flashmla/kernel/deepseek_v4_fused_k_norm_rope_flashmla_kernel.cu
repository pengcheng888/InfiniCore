...
namespace infinicore::op::deepseek_v4_fused_k_norm_rope_flashmla {
namespace {
...
__global__ void fused_k_norm_rope_flashmla_kernel(const void *__restrict__ kv,
    ...
    const Float2 weight = load_bf16_pair(kv_weight, elem);
    ...
    if (warp_id == kRopeWarp) {
        ...
        const float abs_max = warp_max(fmaxf(fabsf(data.x), fabsf(data.y)));
        ...
        store_fp8_pair(value_ptr, tid, data.x * inv_scale, data.y * inv_scale);
        if (lane == 0) {
...
void launch_fused_k_norm_rope_flashmla(const void *kv,
    ...
    if (page_size == 256) {
        fused_k_norm_rope_flashmla_kernel<8><<<static_cast<unsigned int>(tokens), kThreads, 0, cuda_stream>>>(
            kv,
            kv_weight,
            freqs_cis,
            positions,
    ...
    fused_k_norm_rope_flashmla_kernel<-1><<<static_cast<unsigned int>(tokens), kThreads, 0, cuda_stream>>>(
        kv,
        kv_weight,
        freqs_cis,
        positions,
