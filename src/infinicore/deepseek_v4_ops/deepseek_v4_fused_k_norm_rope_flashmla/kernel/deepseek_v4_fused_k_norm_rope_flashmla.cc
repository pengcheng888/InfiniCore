...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(FusedKNormRopeFlashMLA);

FusedKNormRopeFlashMLA::FusedKNormRopeFlashMLA(const Tensor &kv,
                                               const Tensor &kv_weight,
                                               float epsilon,
                                               const Tensor &freqs_cis,
                                               const Tensor &positions,
...
void FusedKNormRopeFlashMLA::execute(const Tensor &kv,
                                     const Tensor &kv_weight,
                                     float epsilon,
                                     const Tensor &freqs_cis,
                                     const Tensor &positions,
                                     ...
                                     Tensor kvcache,
                                     int page_size) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(FusedKNormRopeFlashMLA,
                                      kv,
                                      kv_weight,
                                      epsilon,
                                      freqs_cis,
...
namespace deepseek_v4_fused_k_norm_rope_flashmla_impl {
...
void run(void *planned_meta) {
    ...
    ::infinicore::op::deepseek_v4_fused_k_norm_rope_flashmla::launch_fused_k_norm_rope_flashmla(
        planned->kv->data(),
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(FusedKNormRopeFlashMLA,
                                       &deepseek_v4_fused_k_norm_rope_flashmla_impl::plan,
                                       &deepseek_v4_fused_k_norm_rope_flashmla_impl::run,
                                       &deepseek_v4_fused_k_norm_rope_flashmla_impl::cleanup);
} // namespace deepseek_v4_fused_k_norm_rope_flashmla_register
...
void deepseek_v4_fused_k_norm_rope_flashmla_kernel_(const Tensor &kv,
    ...
    check_fused_k_norm_rope_flashmla_shapes(kv,
                                            ...
                                            page_size,
                                            "deepseek_v4_fused_k_norm_rope_flashmla_kernel_");
    deepseek_v4::FusedKNormRopeFlashMLA::execute(kv, kv_weight, epsilon, freqs_cis, positions, out_loc, kvcache, page_size);
    return;
#else
    (void)kv;
    (void)kv_weight;
