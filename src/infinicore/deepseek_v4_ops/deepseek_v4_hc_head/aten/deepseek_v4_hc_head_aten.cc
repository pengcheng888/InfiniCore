...
void deepseek_v4_hc_head_aten_(Tensor y,
                               ...
                               double hc_eps) {
#if defined(ENABLE_ATEN) && (defined(ENABLE_HYGON_API) || defined(ENABLE_NVIDIA_API))
    check_accelerator_tensor(x, "deepseek_v4_hc_head_aten_");
#if defined(ENABLE_HYGON_API)
    ...
    if (x->ndim() != 3 || fn->ndim() != 2 || scale->ndim() != 1 || base->ndim() != 1) {
        throw std::runtime_error("deepseek_v4_hc_head_aten_ unexpected input rank.");
    }
    ...
    if (fn->shape() != Shape{static_cast<size_t>(hc), static_cast<size_t>(hc * hidden)} || base->size(0) != static_cast<size_t>(hc) || scale->size(0) != 1 || y->shape() != Shape{static_cast<size_t>(tokens), static_cast<size_t>(hidden)}) {
        throw std::runtime_error("deepseek_v4_hc_head_aten_ shape mismatch.");
    }
    ...
    (void)rms_eps;
    (void)hc_eps;
    throw std::runtime_error("deepseek_v4_hc_head_aten_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
...
void deepseek_v4_hc_head_naive_(Tensor y,
                                ...
                                double rms_eps,
                                double hc_eps) {
    deepseek_v4_hc_head_aten_(y, x, fn, scale, base, rms_eps, hc_eps);
}
