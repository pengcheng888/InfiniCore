...
Tensor RowParallelLinear::forward(Tensor &input) const {
    ...
    if ((tp_size_ > 1) && (communicator_ != nullptr)) {
        ...
        Shape shape = output->shape(); // [bs, seq_len, hidden_size]
        Size count = shape[0] * shape[1] * shape[2];

        DataType type = output->dtype();

        infinirtStream_t stream = infinicore::context::getStream();

        infinicclAllReduce(output->data(), output->data(), count, static_cast<infiniDtype_t>(static_cast<int>(type)),
