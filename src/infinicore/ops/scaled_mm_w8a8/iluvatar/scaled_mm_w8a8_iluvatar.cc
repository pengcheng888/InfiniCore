...
namespace infinicore::op::scaled_mm_w8a8_impl::iluvatar {
...
void run(Tensor out,
         ...
         bool trans_weight) {
    if (!adaptor::iluvatar_vendor::scaled_mm_w8a8_available()) {
        throw std::runtime_error("scaled_mm_w8a8 requires the Iluvatar vendor extension");
    }
    ...
    adaptor::iluvatar_vendor::scaled_mm_w8a8(
        out_at, a_at,
        b_at, a_scales_at,
...
static bool registered = []() {
    vendor_ops::scaled_mm_w8a8_dispatcher().registerDevice(Device::Type::ILUVATAR, &run);
    return true;
}();
