...
static bool registered = []() {
    MhaKVCache::plan_dispatcher().registerDevice(Device::Type::MOORE, &plan);
    MhaKVCache::run_dispatcher().registerDevice(Device::Type::MOORE, &run);
    MhaKVCache::cleanup_dispatcher().registerDevice(Device::Type::MOORE, &cleanup);
    return true;
}();
