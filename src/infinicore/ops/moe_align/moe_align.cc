...
MoeAlign::MoeAlign(Tensor sorted_token_ids,
    ...
    INFINICORE_GRAPH_OP_DISPATCH(
        sorted_token_ids->device().getType(),
        sorted_token_ids,
        expert_ids,
...
std::tuple<Tensor, Tensor, Tensor> moe_align(
    ...
    auto sorted_token_ids = Tensor::empty({sorted_token_ids_capacity}, DataType::I32, topk_ids->device());
    auto expert_ids = Tensor::empty({max_num_blocks}, DataType::I32, topk_ids->device());
    auto num_tokens_post_padded = Tensor::empty({1}, DataType::I32, topk_ids->device());
