...
void deepseek_v4_fused_experts_impl_int8_marlin_(Tensor output,
    ...
    if (hidden_states->device().getType() != Device::Type::HYGON || output->device().getType() != Device::Type::HYGON) {
        throw std::runtime_error("deepseek_v4_fused_experts_impl_int8_marlin_ expects HYGON tensors in this build.");
    }
    c10::hip::HIPStreamGuard guard(infinicore::adaptor::get_hip_stream());
#else
    if (hidden_states->device().getType() != Device::Type::NVIDIA || output->device().getType() != Device::Type::NVIDIA) {
        throw std::runtime_error("deepseek_v4_fused_experts_impl_int8_marlin_ expects NVIDIA tensors in this build.");
    }
    ...
    (void)inplace;
    throw std::runtime_error("deepseek_v4_fused_experts_impl_int8_marlin_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
