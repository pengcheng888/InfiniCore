...
namespace infinicore::op::w4a8_group_gemm_impl::iluvatar {
...
void run(Tensor out,
         ...
         bool trans_weight,
         bool is_decode) {
    if (!adaptor::iluvatar_vendor::w4a8_group_gemm_available()) {
        throw std::runtime_error("w4a8_group_gemm requires the Iluvatar vendor extension");
    }
    ...
    adaptor::iluvatar_vendor::w4a8_group_gemm(
        out_at, input_at,
...
static bool registered = []() {
    vendor_ops::w4a8_group_gemm_dispatcher().registerDevice(Device::Type::ILUVATAR, &run);
    return true;
}();
