...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(HcHead);
...
void HcHead::execute(Tensor y,
                     ...
                     double rms_eps,
                     double hc_eps) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(HcHead, y, x, fn, scale, base, rms_eps, hc_eps);
}

namespace hc_head_impl {
...
void *plan_head(Tensor y,
    ...
    int64_t hc = 0;
    int64_t hidden = 0;
    validate_head_kernel_tensors(y, x, fn, scale, base, tokens, hc, hidden, "hc_head_kernel_");
...
#else
    (void)planned_meta;
    throw std::runtime_error("hc_head_kernel_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
...
namespace hc_head_register {
INFINICORE_DSV4_GRAPH_OP_REGISTER_BUILD_DEVICE(
    HcHead,
    &hc_head_impl::plan_head,
    &hc_head_impl::run_head,
    &hc_head_impl::cleanup_head);
} // namespace hc_head_register
...
void hc_head_kernel_(Tensor y,
                     ...
                     double rms_eps,
                     double hc_eps) {
    HcHead::execute(y, x, fn, scale, base, rms_eps, hc_eps);
}
