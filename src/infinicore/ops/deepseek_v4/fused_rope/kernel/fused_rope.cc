...
void check_rope_tensor(const Tensor &tensor, const char *name) {
    if (tensor->ndim() != 2 && tensor->ndim() != 3) {
        throw std::runtime_error(std::string("fused_rope_ expects ") + name + " rank 2 or 3.");
    }
    if (tensor->size(tensor->ndim() - 1) != kDsv4RopeDim) {
        throw std::runtime_error(std::string("fused_rope_ expects ") + name + " last dim 64.");
    }
    if (tensor->dtype() != DataType::BF16) {
        throw std::runtime_error(std::string("fused_rope_ expects ") + name + " dtype bf16.");
    }
    if (tensor->stride(tensor->ndim() - 1) != 1) {
        throw std::runtime_error(std::string("fused_rope_ expects ") + name + " last-dim stride 1.");
    }
}
...
int dsv4_scalar_type_for_kernel(const Tensor &tensor, const char *op_name) {
    if (tensor->dtype() == DataType::BF16) {
        return fused_rope::kDsv4BF16;
    }
    if (tensor->dtype() == DataType::F16) {
        return fused_rope::kDsv4F16;
    }
    if (tensor->dtype() == DataType::F32) {
        return fused_rope::kDsv4F32;
    }
...
void *plan(Tensor query,
    ...
    if (key.has_value()) {
        key_meta = make_tensor_meta(key.value(), "fused_rope_");
    }
    return new PlannedMeta{make_tensor_meta(query, "fused_rope_"),
                           std::move(key_meta),
                           graph::GraphTensor(freqs_cis),
                           graph::GraphTensor(positions),
...
#else
    (void)planned_meta;
    throw std::runtime_error("fused_rope_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
...
namespace fused_rope_register {
INFINICORE_DSV4_GRAPH_OP_REGISTER_BUILD_DEVICE(
    FusedRope,
    &fused_rope_impl::plan,
    &fused_rope_impl::run,
    &fused_rope_impl::cleanup);
} // namespace fused_rope_register

void fused_rope_kernel_(Tensor query,
                        std::optional<Tensor> key,
                        const Tensor &freqs_cis,
                        const Tensor &positions,
                        bool inverse) {
