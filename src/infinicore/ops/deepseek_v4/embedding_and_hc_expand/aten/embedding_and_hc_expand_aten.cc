...
Shape output_shape_for_aten(const Tensor &input, const Tensor &weight, int64_t hc_mult, const char *op_name) {
    if (weight->ndim() != 2) {
...
void check_shapes_and_dtypes_aten(const Tensor &out, const Tensor &input, const Tensor &weight, int64_t hc_mult, const char *op_name) {
    auto expected = output_shape_for_aten(input, weight, hc_mult, op_name);
...
void check_same_device_aten(const Tensor &out, const Tensor &input, const Tensor &weight, const char *op_name) {
...
void check_accelerator_tensor_aten(const Tensor &tensor, const char *op_name) {
...
Tensor embedding_and_hc_expand_aten(const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    auto out = Tensor::empty(output_shape_for_aten(input, weight, hc_mult, "embedding_and_hc_expand_aten"), weight->dtype(), weight->device());
    embedding_and_hc_expand_aten_(out, input, weight, hc_mult);
    return out;
}
...
void embedding_and_hc_expand_aten_(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    ...
    check_accelerator_tensor_aten(out, "embedding_and_hc_expand_aten_");
    detail::prepare_aten_call(out, "embedding_and_hc_expand_aten_");

    check_same_device_aten(out, input, weight, "embedding_and_hc_expand_aten_");
    check_shapes_and_dtypes_aten(out, input, weight, hc_mult, "embedding_and_hc_expand_aten_");
    ...
    (void)weight;
    (void)hc_mult;
    throw std::runtime_error("embedding_and_hc_expand_aten_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
