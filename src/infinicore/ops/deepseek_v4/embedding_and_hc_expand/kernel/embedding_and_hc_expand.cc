...
#include "embedding_and_hc_expand_kernel.hpp"
...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(EmbeddingAndHcExpand);
...
EmbeddingAndHcExpand::EmbeddingAndHcExpand(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    INFINICORE_ASSERT_TENSORS_SAME_DEVICE(out, input, weight);
    INFINICORE_GRAPH_OP_DISPATCH(out->device().getType(), out, input, weight, hc_mult);
}

void EmbeddingAndHcExpand::execute(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(EmbeddingAndHcExpand, out, input, weight, hc_mult);
}

namespace embedding_and_hc_expand_impl {
...
namespace embedding_and_hc_expand_kernel_register {
INFINICORE_DSV4_GRAPH_OP_REGISTER_BUILD_DEVICE(
    EmbeddingAndHcExpand,
    &embedding_and_hc_expand_impl::plan,
    &embedding_and_hc_expand_impl::run,
    &embedding_and_hc_expand_impl::cleanup);
} // namespace embedding_and_hc_expand_kernel_register

Tensor embedding_and_hc_expand_kernel(const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    auto out = Tensor::empty(output_shape_for_kernel(input, weight, hc_mult, "embedding_and_hc_expand_kernel"), weight->dtype(), weight->device());
    embedding_and_hc_expand_kernel_(out, input, weight, hc_mult);
    return out;
}
...
void embedding_and_hc_expand_kernel_(Tensor out, const Tensor &input, const Tensor &weight, int64_t hc_mult) {
    ...
    (void)weight;
    (void)hc_mult;
    throw std::runtime_error("embedding_and_hc_expand_kernel_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
