...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(MhcPost);
...
void MhcPost::execute(Tensor y,
                                const Tensor &x,
                                const Tensor &residual,
                                const Tensor &post,
                                const Tensor &comb) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(MhcPost, y, x, residual, post, comb);
}

namespace mhc_post_impl {
...
void *plan_post(Tensor y,
    ...
    int64_t hc = 0;
    int64_t hidden = 0;
    validate_post_kernel_tensors(y, x, residual, post, comb, tokens, hc, hidden, "mhc_post_kernel_");
...
#else
    (void)planned_meta;
    throw std::runtime_error("mhc_post_kernel_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
...
namespace mhc_post_register {
INFINICORE_DSV4_GRAPH_OP_REGISTER_BUILD_DEVICE(
    MhcPost,
    &mhc_post_impl::plan_post,
    &mhc_post_impl::run_post,
    &mhc_post_impl::cleanup_post);
} // namespace mhc_post_register
...
void mhc_post_kernel_(Tensor y,
                                  ...
                                  const Tensor &post,
                                  const Tensor &comb) {
    MhcPost::execute(y, x, residual, post, comb);
}
