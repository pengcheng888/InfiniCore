...
void moe_w8a8_aten_(Tensor y,
    ...
    check_accelerator_tensor(x, "deepseek_v4::moe_w8a8_aten_");
    detail::prepare_aten_call(x, "deepseek_v4::moe_w8a8_aten_");

    if (x->ndim() != 2 || topk_weights->ndim() != 2 || topk_indices->ndim() != 2 ||
        w13->ndim() != 3 || w13_scale->ndim() != 3 || w2->ndim() != 3 || w2_scale->ndim() != 3 ||
        y->shape() != x->shape()) {
        throw std::runtime_error("deepseek_v4::moe_w8a8_aten_ shape/rank mismatch.");
    }
    if (topk_weights->shape() != topk_indices->shape()) {
        throw std::runtime_error("deepseek_v4::moe_w8a8_aten_ topk shape mismatch.");
    }
    if (w13->size(0) != w2->size(0) || w13->size(1) != w2->size(2) * 2 ||
        w13->size(2) != x->size(1) || w2->size(1) != x->size(1)) {
        throw std::runtime_error("deepseek_v4::moe_w8a8_aten_ packed weight shape mismatch.");
    }
    ...
    (void)w2_scale;
    (void)swiglu_limit;
    throw std::runtime_error("deepseek_v4::moe_w8a8_aten_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
