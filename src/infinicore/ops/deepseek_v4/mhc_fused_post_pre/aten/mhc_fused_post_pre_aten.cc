...
void mhc_fused_post_pre_aten_(Tensor residual_cur,
                              Tensor post_mix_cur,
                              Tensor comb_mix_cur,
    ...
    if (hc_post_mult_value != 2.0) {
        throw std::runtime_error("mhc_fused_post_pre_aten_ currently expects hc_post_mult_value == 2.0.");
    }
    mhc_post_aten_(residual_cur, x, residual, post_layer_mix, comb_res_mix);
    mhc_pre_aten_(layer_input_cur,
                  post_mix_cur,
                  comb_mix_cur,
                  residual_cur,
                  fn,
                  ...
                  hc_sinkhorn_eps,
                  sinkhorn_repeat);
    rms_norm_(layer_input_cur, layer_input_cur, norm_weight, static_cast<float>(norm_eps));
}
