...
#include "mhc_fused_post_pre_kernel.hpp"
...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(MhcFusedPostPre);
...
MhcFusedPostPre::MhcFusedPostPre(Tensor residual_cur,
                                                     Tensor post_mix_cur,
                                                     Tensor comb_mix_cur,
...
void MhcFusedPostPre::execute(Tensor residual_cur,
                                        Tensor post_mix_cur,
                                        Tensor comb_mix_cur,
                                        Tensor layer_input_cur,
                                        const Tensor &x,
                                        ...
                                        const Tensor &norm_weight,
                                        double norm_eps) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(MhcFusedPostPre,
                                      residual_cur,
                                      post_mix_cur,
                                      comb_mix_cur,
                                      layer_input_cur,
...
namespace mhc_fused_post_pre_impl {
...
void run(void *planned_meta) {
    ...
    mhc_fused_post_pre::launch_kernel(
...
#else
    (void)planned_meta;
    throw std::runtime_error("mhc_fused_post_pre_kernel_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
...
namespace mhc_fused_post_pre_register {
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(MhcFusedPostPre,
                                       &mhc_fused_post_pre_impl::plan,
                                       &mhc_fused_post_pre_impl::run,
                                       &mhc_fused_post_pre_impl::cleanup);
} // namespace mhc_fused_post_pre_register
...
// ```
void mhc_fused_post_pre_kernel_(Tensor residual_cur,
                                            Tensor post_mix_cur,
                                            Tensor comb_mix_cur,
                                            Tensor layer_input_cur,
                                            const Tensor &x,
    ...
    if (!use_hc4_hidden4096_fused_path(residual)) {
        throw std::runtime_error("mhc_fused_post_pre_kernel_ expects standard fused shape [tokens, 4, 4096].");
    }

    MhcFusedPostPre::execute(residual_cur,
                                                    post_mix_cur,
                                                    comb_mix_cur,
                                                    layer_input_cur,
                                                    x,
