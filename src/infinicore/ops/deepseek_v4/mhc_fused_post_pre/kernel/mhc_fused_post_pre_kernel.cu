...
namespace infinicore::op::deepseek_v4::mhc_fused_post_pre {
namespace {
...
void launch_kernel(void *residual_cur,
    ...
    if (hc_post_mult_value != 2.0) {
        throw std::runtime_error("mhc_fused_post_pre kernel currently expects hc_post_mult_value == 2.0.");
    }
    if (tokens == 0) {
        return;
    }
    ...
    throw std::runtime_error("mhc_fused_post_pre has no fused kernel for this shape.");
}
