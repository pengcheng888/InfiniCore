...
#include "biased_topk_kernel.hpp"
...
INFINICORE_GRAPH_OP_CLASS(TopkDsv4Kernel, Tensor, Tensor, const Tensor &, const Tensor &, bool);
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(TopkDsv4Kernel);
...
void check_shapes(const Tensor &topk_weights,
    ...
    if (router_logits->ndim() != 2) {
        throw std::runtime_error("topk_kernel_ expects router_logits to be 2-D.");
    }
    if (correction_bias->ndim() != 1 || correction_bias->size(0) != router_logits->size(1)) {
        throw std::runtime_error("topk_kernel_ correction_bias shape mismatch.");
    }
    if (topk_weights->shape() != topk_indices->shape()) {
        throw std::runtime_error("topk_kernel_ topk weight/index shape mismatch.");
    }
    if (topk_weights->ndim() != 2 || topk_weights->size(0) != router_logits->size(0)) {
        throw std::runtime_error("topk_kernel_ output shape mismatch.");
    }
}
...
void check_kernel_tensors(const Tensor &topk_weights,
    ...
    if (topk_weights->dtype() != DataType::F32 || topk_indices->dtype() != DataType::I32 ||
        ...
        throw std::runtime_error("topk_kernel_ expects F32 weights/logits/bias and I32 indices.");
    }
    if (!topk_weights->is_contiguous() || !topk_indices->is_contiguous() ||
        !router_logits->is_contiguous() || !correction_bias->is_contiguous()) {
        throw std::runtime_error("topk_kernel_ expects contiguous tensors.");
    }
}
...
TopkDsv4Kernel::TopkDsv4Kernel(Tensor topk_weights,
                                                   Tensor topk_indices,
                                                   const Tensor &router_logits,
                                                   const Tensor &correction_bias,
                                                   bool renormalize) {
...
void TopkDsv4Kernel::execute(Tensor topk_weights,
                                       Tensor topk_indices,
                                       const Tensor &router_logits,
                                       const Tensor &correction_bias,
                                       bool renormalize) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(TopkDsv4Kernel, topk_weights, topk_indices, router_logits, correction_bias, renormalize);
}

namespace biased_topk_graph_impl {
...
void *plan_dsv4(Tensor topk_weights,
                ...
                const Tensor &correction_bias,
                bool renormalize) {
    check_accelerator_tensor(router_logits, "TopkDsv4Kernel");
    ...
    if (!is_dsv4_topk_shape(topk_weights, router_logits, renormalize)) {
        throw std::runtime_error("TopkDsv4Kernel requires num_experts=256, topk=6, renormalize=true.");
    }
...
namespace biased_topk_register {
INFINICORE_DSV4_NATIVE_GRAPH_OP_REGISTER_BUILD_DEVICE(
    TopkDsv4Kernel,
    &biased_topk_graph_impl::plan_dsv4,
    &biased_topk_graph_impl::run_dsv4,
    &biased_topk_graph_impl::cleanup_dsv4);
} // namespace biased_topk_register
...
void run_biased_topk_dsv4_kernel(Tensor topk_weights,
                                 ...
                                 const Tensor &correction_bias,
                                 bool renormalize) {
    TopkDsv4Kernel::execute(topk_weights, topk_indices, router_logits, correction_bias, renormalize);
}
...
void topk_generic_kernel_(Tensor topk_weights,
    ...
    check_kernel_tensors(topk_weights, topk_indices, router_logits, correction_bias);
    if (router_logits->size(1) > 512 || topk_weights->size(1) > 16) {
        throw std::runtime_error("topk_generic_kernel_ supports num_experts <= 512 and topk <= 16.");
    }

    biased_topk::launch_biased_topk_generic(
        reinterpret_cast<float *>(topk_weights->data()),
        reinterpret_cast<int32_t *>(topk_indices->data()),
        reinterpret_cast<const float *>(router_logits->data()),
        reinterpret_cast<const float *>(correction_bias->data()),
