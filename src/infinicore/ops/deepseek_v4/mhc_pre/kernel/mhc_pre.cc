...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(MhcPre);
...
void MhcPre::execute(Tensor y,
                     ...
                     double hc_sinkhorn_eps,
                     int sinkhorn_repeat) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(MhcPre,
                                      y,
                                      post,
                                      comb,
                                      residual,
...
void validate_pre_kernel_tensors(Tensor y,
    ...
    (void)hc;
    (void)hidden;
    throw std::runtime_error(std::string(op_name) + " requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
...
void *plan_pre(Tensor y,
    ...
    int64_t hc = 0;
    int64_t hidden = 0;
    validate_pre_kernel_tensors(y, post, comb, residual, fn, hc_scale, hc_base, tokens, hc, hidden, "mhc_pre_kernel_");
    const int64_t mix_hc = (2 + hc) * hc;
...
#else
    (void)planned_meta;
    throw std::runtime_error("mhc_pre_kernel_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
...
namespace mhc_pre_register {
INFINICORE_DSV4_GRAPH_OP_REGISTER_BUILD_DEVICE(
    MhcPre,
    &mhc_pre_impl::plan_pre,
    &mhc_pre_impl::run_pre,
    &mhc_pre_impl::cleanup_pre);
} // namespace mhc_pre_register

void mhc_pre_kernel_(Tensor y,
                     Tensor post,
                     Tensor comb,
                     const Tensor &residual,
                     const Tensor &fn,
                     ...
                     double hc_sinkhorn_eps,
                     int sinkhorn_repeat) {
    MhcPre::execute(y, post, comb, residual, fn, hc_scale, hc_base, rms_eps, hc_pre_eps, hc_sinkhorn_eps, sinkhorn_repeat);
}
