...
void mhc_pre_aten_(Tensor y,
                   ...
                   int sinkhorn_repeat) {
#if defined(ENABLE_ATEN) && defined(INFINICORE_DSV4_ACCELERATOR_API)
    check_accelerator_tensor(residual, "mhc_pre_aten_");
    detail::prepare_aten_call(residual, "mhc_pre_aten_");

    if (residual->ndim() != 3 || fn->ndim() != 2 || hc_scale->ndim() != 1 || hc_base->ndim() != 1) {
        throw std::runtime_error("mhc_pre_aten_ unexpected input rank.");
    }
    ...
    if (fn->shape() != Shape{static_cast<size_t>(mix_hc), static_cast<size_t>(hc * hidden)}
        ...
        throw std::runtime_error("mhc_pre_aten_ shape mismatch.");
    }
    ...
    throw std::runtime_error("mhc_pre_aten_ requires an ATen-enabled HYGON/NVIDIA/METAX/ILUVATAR build.");
#endif
}
