...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(DeepseekV4IndexerCompressNormRopeStoreKernel);

namespace {

constexpr int64_t kHeadDim = 128;
constexpr int64_t kRopeDim = 64;
constexpr int64_t kValueBytesPerToken = 128;
constexpr int64_t kScaleBytesPerToken = 4;
constexpr int kDsv4BF16 = 0;
constexpr int kDsv4F32 = 2;

int64_t indexer_page_bytes(int page_size) {
    return (kValueBytesPerToken + kScaleBytesPerToken) * static_cast<int64_t>(page_size);
}
...
int norm_weight_dtype_for_kernel(const Tensor &norm_weight, const char *op_name) {
    if (norm_weight->dtype() == DataType::BF16) {
        return kDsv4BF16;
    }
    if (norm_weight->dtype() == DataType::F32) {
        return kDsv4F32;
    }
    throw std::runtime_error(std::string(op_name) + " supports bf16/fp32 norm_weight only.");
}

void check_indexer_compress_norm_rope_store_shapes(const Tensor &kv,
    ...
    if (kv->ndim() != 2 || kv->size(1) != static_cast<size_t>(kHeadDim)) {
        throw std::runtime_error(std::string(op_name) + " expects kv [tokens, 128].");
    }
    if (kv->dtype() != DataType::BF16) {
        throw std::runtime_error(std::string(op_name) + " supports bf16 kv only.");
    }
    if (kv->stride(1) != 1) {
        throw std::runtime_error(std::string(op_name) + " expects contiguous head dimension in kv.");
    }
    if (norm_weight->ndim() != 1 || norm_weight->size(0) != static_cast<size_t>(kHeadDim) ||
        !norm_weight->is_contiguous()) {
        throw std::runtime_error(std::string(op_name) + " expects contiguous norm_weight [128].");
    }
    norm_weight_dtype_for_kernel(norm_weight, op_name);
    ...
    if (kvcache->size(1) != static_cast<size_t>(indexer_page_bytes(page_size))) {
...
DeepseekV4IndexerCompressNormRopeStoreKernel::DeepseekV4IndexerCompressNormRopeStoreKernel(
    const Tensor &kv,
    const Tensor &norm_weight,
    float epsilon,
    const Tensor &freqs_cis,
...
void DeepseekV4IndexerCompressNormRopeStoreKernel::execute(const Tensor &kv,
                                                           const Tensor &norm_weight,
                                                           float epsilon,
                                                           const Tensor &freqs_cis,
                                                           const Tensor &positions,
                                                           ...
                                                           Tensor kvcache,
                                                           int page_size) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(DeepseekV4IndexerCompressNormRopeStoreKernel,
                                      kv,
                                      norm_weight,
                                      epsilon,
                                      freqs_cis,
...
namespace deepseek_v4_indexer_compress_norm_rope_store_graph_impl {
...
void *plan(const Tensor &kv,
           ...
           Tensor kvcache,
           int page_size) {
    check_indexer_compress_norm_rope_store_shapes(kv,
                                                  norm_weight,
                                                  freqs_cis,
                                                  positions,
                                                  out_loc,
                                                  kvcache,
                                                  page_size,
                                                  "deepseek_v4_indexer_compress_norm_rope_store_kernel_");
    return new PlannedMeta{graph::GraphTensor(kv),
                           ...
                           graph::GraphTensor(out_loc),
                           graph::GraphTensor(kvcache),
                           norm_weight_dtype_for_kernel(norm_weight, "deepseek_v4_indexer_compress_norm_rope_store_kernel_"),
                           positions->dtype() == DataType::I64,
                           out_loc->dtype() == DataType::I64,
                           static_cast<int64_t>(kv->size(0)),
                           static_cast<int64_t>(kv->stride(0)),
                           page_size,
                           indexer_page_bytes(page_size),
                           epsilon};
}
...
void run(void *planned_meta) {
    ...
    deepseek_v4_indexer_compress_norm_rope_store_native::launch_indexer_compress_norm_rope_store(
        planned->kv->data(),
...
#else
    (void)planned_meta;
    throw std::runtime_error("deepseek_v4_indexer_compress_norm_rope_store_kernel_ requires a HYGON/NVIDIA build.");
#endif
}
...
namespace deepseek_v4_indexer_compress_norm_rope_store_register {
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(
    DeepseekV4IndexerCompressNormRopeStoreKernel,
    &deepseek_v4_indexer_compress_norm_rope_store_graph_impl::plan,
    &deepseek_v4_indexer_compress_norm_rope_store_graph_impl::run,
    &deepseek_v4_indexer_compress_norm_rope_store_graph_impl::cleanup);
} // namespace deepseek_v4_indexer_compress_norm_rope_store_register
...
void deepseek_v4_indexer_compress_norm_rope_store_kernel_(const Tensor &kv,
    ...
    check_indexer_compress_norm_rope_store_shapes(kv,
                                                  ...
                                                  page_size,
                                                  "deepseek_v4_indexer_compress_norm_rope_store_kernel_");
    DeepseekV4IndexerCompressNormRopeStoreKernel::execute(kv,
                                                          norm_weight,
                                                          epsilon,
                                                          freqs_cis,
                                                          positions,
