...
void check_rope_tensor(const Tensor &tensor, const char *name) {
    if (tensor->ndim() != 2 && tensor->ndim() != 3) {
        throw std::runtime_error(std::string("deepseek_v4_fused_rope_naive_ expects ") + name + " rank 2 or 3.");
    }
    if (tensor->size(tensor->ndim() - 1) != static_cast<size_t>(kDsv4RopeDim)) {
        throw std::runtime_error(std::string("deepseek_v4_fused_rope_naive_ expects ") + name + " last dim 64.");
    }
    if (tensor->dtype() != DataType::BF16) {
        throw std::runtime_error(std::string("deepseek_v4_fused_rope_naive_ expects ") + name + " dtype bf16.");
    }
}
...
void apply_rope_aten_(at::Tensor x, const at::Tensor &freqs_cis, const at::Tensor &positions, bool inverse) {
    if (x.dim() != 2 && x.dim() != 3) {
        throw std::runtime_error("deepseek_v4_fused_rope_naive_ ATen tensor rank must be 2 or 3.");
    }
    if (x.size(-1) != kDsv4RopeDim) {
        throw std::runtime_error("deepseek_v4_fused_rope_naive_ ATen tensor last dim must be 64.");
    }
    const int64_t batch = x.size(0);
    if (positions.numel() != batch) {
        throw std::runtime_error("deepseek_v4_fused_rope_naive_ positions length mismatch.");
    }
    if (batch == 0) {
        return;
    }
...
void deepseek_v4_fused_rope_naive_(Tensor query,
...
#endif
    check_tensor_device(query, "deepseek_v4_fused_rope_naive_");
    check_tensor_device(freqs_cis, "deepseek_v4_fused_rope_naive_");
    check_tensor_device(positions, "deepseek_v4_fused_rope_naive_");
    check_rope_tensor(query, "query");
    if (key.has_value()) {
        check_tensor_device(key.value(), "deepseek_v4_fused_rope_naive_");
        check_rope_tensor(key.value(), "key");
        if (key.value()->size(0) != query->size(0)) {
            throw std::runtime_error("deepseek_v4_fused_rope_naive_ key/query batch mismatch.");
        }
    }
    if (freqs_cis->ndim() != 2 || freqs_cis->size(1) != static_cast<size_t>(kDsv4RopeDim) || freqs_cis->dtype() != DataType::F32) {
        throw std::runtime_error("deepseek_v4_fused_rope_naive_ expects freqs_cis [max_pos, 64] float32.");
    }
    if (positions->ndim() != 1 || (positions->dtype() != DataType::I32 && positions->dtype() != DataType::I64)) {
        throw std::runtime_error("deepseek_v4_fused_rope_naive_ expects positions [tokens] int32/int64.");
    }
    ...
    (void)positions;
    (void)inverse;
    throw std::runtime_error("deepseek_v4_fused_rope_naive_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
