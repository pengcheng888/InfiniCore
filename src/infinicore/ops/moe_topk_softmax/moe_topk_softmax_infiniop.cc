...
namespace infinicore::op::moe_topk_softmax_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, MoeTopkSoftmax, 100);
...
void *plan(Tensor topk_weights,
    ...
    size_t seed = hash_combine(topk_weights, topk_indices, gating_output, correction_bias, renormalize, moe_softcapping);
    ...
    INFINIOP_CACHABLE_DESCRIPTOR_GET_OR_CREATE(
        ...
        renormalize,
        moe_softcapping);
    INFINIOP_WORKSPACE_TENSOR(workspace, MoeTopkSoftmax, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopMoeTopkSoftmax(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(MoeTopkSoftmax, &plan, &run, cleanup);
