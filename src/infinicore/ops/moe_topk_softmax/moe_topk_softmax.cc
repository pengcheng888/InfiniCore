...
std::tuple<Tensor, Tensor> moe_topk_softmax(
    ...
    auto topk_weights = Tensor::empty({shape[0], topk}, DataType::F32, gating_output->device());
    auto topk_indices = Tensor::empty({shape[0], topk}, DataType::I32, gating_output->device());
