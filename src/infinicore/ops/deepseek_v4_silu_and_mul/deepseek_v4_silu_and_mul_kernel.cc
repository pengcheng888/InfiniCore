...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(DeepseekV4SiluAndMulKernel);
...
DeepseekV4SiluAndMulKernel::DeepseekV4SiluAndMulKernel(Tensor out, const Tensor &x) {
    INFINICORE_ASSERT_TENSORS_SAME_DEVICE(out, x);
    INFINICORE_GRAPH_OP_DISPATCH(out->device().getType(), out, x);
}

void DeepseekV4SiluAndMulKernel::execute(Tensor out, const Tensor &x) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(DeepseekV4SiluAndMulKernel, out, x);
}

namespace deepseek_v4_silu_and_mul_graph_impl {
...
void run(void *planned_meta) {
    ...
    deepseek_v4_silu_and_mul_impl::launch_silu_and_mul(
        planned->out->data(),
        planned->x->data(),
        planned->tokens,
        planned->hidden,
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(DeepseekV4SiluAndMulKernel,
                                       &deepseek_v4_silu_and_mul_graph_impl::plan,
                                       &deepseek_v4_silu_and_mul_graph_impl::run,
                                       &deepseek_v4_silu_and_mul_graph_impl::cleanup);
} // namespace deepseek_v4_silu_and_mul_register

void deepseek_v4_silu_and_mul_kernel_(Tensor out, const Tensor &x) {
#if defined(ENABLE_HYGON_API) || defined(ENABLE_NVIDIA_API)
    check_kernel_tensors(out, x, "deepseek_v4_silu_and_mul_kernel_");
    DeepseekV4SiluAndMulKernel::execute(out, x);
#else
    (void)out;
    (void)x;
    throw std::runtime_error("deepseek_v4_silu_and_mul_kernel_ requires an ATen-enabled HYGON/NVIDIA build.");
