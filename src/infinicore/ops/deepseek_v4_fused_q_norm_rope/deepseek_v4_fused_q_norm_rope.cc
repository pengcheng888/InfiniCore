...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(DeepseekV4FusedQNormRopeKernel);
...
DeepseekV4FusedQNormRopeKernel::DeepseekV4FusedQNormRopeKernel(Tensor q_out,
                                                               const Tensor &q_input,
                                                               float epsilon,
                                                               const Tensor &freqs_cis,
                                                               const Tensor &positions) {
...
void DeepseekV4FusedQNormRopeKernel::execute(Tensor q_out,
                                             const Tensor &q_input,
                                             float epsilon,
                                             const Tensor &freqs_cis,
                                             const Tensor &positions) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(DeepseekV4FusedQNormRopeKernel,
                                      q_out, q_input, epsilon, freqs_cis, positions);
}

namespace deepseek_v4_fused_q_norm_rope_graph_impl {
...
void run(void *planned_meta) {
    ...
    deepseek_v4_fused_q_norm_rope_native::launch_fused_q_norm_rope(planned->q_out->data(),
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(DeepseekV4FusedQNormRopeKernel,
                                       &deepseek_v4_fused_q_norm_rope_graph_impl::plan,
                                       &deepseek_v4_fused_q_norm_rope_graph_impl::run,
                                       &deepseek_v4_fused_q_norm_rope_graph_impl::cleanup);
} // namespace deepseek_v4_fused_q_norm_rope_register
...
#if defined(ENABLE_HYGON_API) || defined(ENABLE_NVIDIA_API)
    check_q_norm_rope_shapes(q_out, q_input, freqs_cis, positions, "deepseek_v4_fused_q_norm_rope_kernel_");
    DeepseekV4FusedQNormRopeKernel::execute(q_out, q_input, epsilon, freqs_cis, positions);
#else
    (void)q_out;
    (void)q_input;
    (void)epsilon;
