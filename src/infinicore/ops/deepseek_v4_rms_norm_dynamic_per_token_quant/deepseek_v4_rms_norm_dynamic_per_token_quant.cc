...
void deepseek_v4_rms_norm_dynamic_per_token_quant_(Tensor result,
    ...
    if (input->device().getType() != Device::Type::HYGON) {
        throw std::runtime_error("deepseek_v4_rms_norm_dynamic_per_token_quant_ expects HYGON tensors in this build.");
    }
    c10::hip::HIPStreamGuard guard(infinicore::adaptor::get_hip_stream());
#else
    if (input->device().getType() != Device::Type::NVIDIA) {
        throw std::runtime_error("deepseek_v4_rms_norm_dynamic_per_token_quant_ expects NVIDIA tensors in this build.");
    }
    ...
    static auto op = c10::Dispatcher::singleton()
                         .findSchemaOrThrow("_C::rms_norm_dynamic_per_token_quant", "")
                         .typed<void(at::Tensor &, const at::Tensor &, const at::Tensor &, at::Tensor &, double, std::optional<at::Tensor>, std::optional<at::Tensor>)>();
    op.call(result_at, input_at, weight_at, scale_at, static_cast<double>(epsilon), scale_ub_at, residual_at);
#else
    (void)result;
    (void)input;
    (void)weight;
    ...
    (void)scale_ub;
    (void)residual;
    throw std::runtime_error("deepseek_v4_rms_norm_dynamic_per_token_quant_ requires an ATen-enabled HYGON/NVIDIA build.");
#endif
}
