...
PrepareMoeInputOutput prepare_moe_input(
    ...
    auto expert_offsets = Tensor::empty({num_experts + 1}, DataType::I32, topk_ids->device());
    auto problem_sizes1 = Tensor::empty({num_experts, 3}, DataType::I32, topk_ids->device());
    auto problem_sizes2 = Tensor::empty({num_experts, 3}, DataType::I32, topk_ids->device());
    auto input_permutation = Tensor::empty({topk_length}, DataType::I32, topk_ids->device());
    auto output_permutation = Tensor::empty({topk_length}, DataType::I32, topk_ids->device());
