...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(DeepseekV4RmsnormSelfKernel);
...
DeepseekV4RmsnormSelfKernel::DeepseekV4RmsnormSelfKernel(Tensor out, const Tensor &x, float epsilon) {
    if (out->device().getType() != x->device().getType() || out->device().getIndex() != x->device().getIndex()) {
...
void DeepseekV4RmsnormSelfKernel::execute(Tensor out, const Tensor &x, float epsilon) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(DeepseekV4RmsnormSelfKernel, out, x, epsilon);
}

namespace deepseek_v4_rmsnorm_self_graph_impl {
...
void run(void *planned_meta) {
    ...
    deepseek_v4_rmsnorm_self_native::launch_rmsnorm_self(planned->out->data(),
                                                         planned->x->data(),
                                                         planned->dtype,
                                                         planned->rows,
                                                         planned->dim,
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(DeepseekV4RmsnormSelfKernel,
                                       &deepseek_v4_rmsnorm_self_graph_impl::plan,
                                       &deepseek_v4_rmsnorm_self_graph_impl::run,
                                       &deepseek_v4_rmsnorm_self_graph_impl::cleanup);
} // namespace deepseek_v4_rmsnorm_self_register
...
void deepseek_v4_rmsnorm_self_kernel_(Tensor out, const Tensor &x, float epsilon) {
    ...
    DeepseekV4RmsnormSelfKernel::execute(out, x, epsilon);
#else
    (void)out;
    (void)x;
    (void)epsilon;
...
Tensor deepseek_v4_rmsnorm_self(const Tensor &x, float epsilon) {
    return deepseek_v4_rmsnorm_self_kernel(x, epsilon);
}
