...
void ensure_flashmla_registered() {
    static bool loaded = []() {
        ...
        if (handle == nullptr) {
            ...
            throw std::runtime_error(
                std::string("flash_mla_with_kvcache_impl requires vllm._flashmla_C; failed to dlopen ")
                + so_path
...
void check_device(const Tensor &tensor, const char *op_name) {
    if (!tensor || tensor->device().getType() != Device::Type::NVIDIA) {
        throw std::runtime_error(std::string(op_name) + " expects NVIDIA tensors.");
    }
}
...
void check_nvidia_dense_decode_options(const std::optional<Tensor> &block_table,
    ...
    if (!has_tensor(block_table) || !has_tensor(cache_seqlens)) {
        throw std::runtime_error(std::string(op_name) + " requires block_table and cache_seqlens on NVIDIA dense decode.");
    }
    if (is_fp8_kvcache) {
        throw std::runtime_error(std::string(op_name) + " does not support is_fp8_kvcache=true on NVIDIA dense decode.");
    }
    if (has_tensor(num_splits)) {
        throw std::runtime_error(std::string(op_name) + " does not support the num_splits override on NVIDIA dense decode.");
    }
    if (has_tensor(indices) || has_tensor(attn_sink) || has_tensor(extra_k_cache)
        ...
        throw std::runtime_error(std::string(op_name) + " currently supports dense attention only on NVIDIA.");
    }
}
...
namespace flash_mla::flash_mla_with_kvcache_nvidia {
...
void *plan(Tensor out,
           ...
           std::optional<Tensor> topk_length,
           std::optional<Tensor> extra_topk_length) {
    check_nvidia_dense_decode_options(block_table,
                                      cache_seqlens,
                                      num_splits,
                                      is_fp8_kvcache,
                                      indices,
...
static bool registered = []() {
    ::infinicore::op::flash_mla::FlashMlaWithKvcache::plan_dispatcher().registerDevice(Device::Type::NVIDIA, &plan);
    ::infinicore::op::flash_mla::FlashMlaWithKvcache::run_dispatcher().registerDevice(Device::Type::NVIDIA, &run);
    ::infinicore::op::flash_mla::FlashMlaWithKvcache::cleanup_dispatcher().registerDevice(Device::Type::NVIDIA, &cleanup);
    ::infinicore::op::flash_mla::flash_mla_with_kvcache_impl_dispatcher().registerDevice(Device::Type::NVIDIA, &flash_mla_with_kvcache_impl);
    return true;
}();
...
void flash_mla_with_kvcache_impl_internal(
    ...
    if (cache_seqlens_tensor->dtype() != DataType::I32 || block_table_tensor->dtype() != DataType::I32) {
        throw std::runtime_error(std::string(op_name) + " expects int32 cache_seqlens and block_table.");
    }
    if (!q->is_contiguous() || !k_cache->is_contiguous()
        || !cache_seqlens_tensor->is_contiguous() || !block_table_tensor->is_contiguous()) {
        throw std::runtime_error(std::string(op_name) + " expects contiguous q/k_cache/cache_seqlens/block_table.");
    }

    c10::cuda::CUDAStreamGuard guard(infinicore::adaptor::get_cuda_stream());

    auto q_at = to_aten_tensor_for_flashmla(q);
    auto k_cache_at = to_aten_tensor_for_flashmla(k_cache);
    auto cache_seqlens_at = to_aten_tensor_for_flashmla(cache_seqlens_tensor);
    auto block_table_at = to_aten_tensor_for_flashmla(block_table_tensor);
    std::optional<at::Tensor> tile_scheduler_metadata_at;
    std::optional<at::Tensor> num_splits_at;
    if (sched_tile_metadata) {
        tile_scheduler_metadata_at = to_aten_tensor_for_flashmla(sched_tile_metadata);
        num_splits_at = to_aten_tensor_for_flashmla(sched_num_splits);
    }
    std::optional<at::Tensor> out_at = to_aten_tensor_for_flashmla(out);
    ...
    const bool has_new_tile_scheduler_metadata = new_tile_scheduler_metadata.has_value()
                                              && new_tile_scheduler_metadata.value().defined();
    const bool has_new_num_splits = new_num_splits.has_value()
                                 && new_num_splits.value().defined();
    if (update_sched_meta && has_new_tile_scheduler_metadata != has_new_num_splits) {
        throw std::runtime_error(std::string(op_name) + " expects FlashMLA scheduler metadata and num_splits to both be set or both be empty.");
    }
    if (update_sched_meta && !has_new_tile_scheduler_metadata) {
        throw std::runtime_error(std::string(op_name) + " expects FlashMLA to return scheduler metadata.");
    }

    if (update_sched_meta) {
        ...
        if (new_tile_scheduler_metadata_tensor->dtype() != DataType::I32
            || new_num_splits_tensor->dtype() != DataType::I32) {
            throw std::runtime_error(std::string(op_name) + " expects FlashMLA scheduler metadata tensors to be int32.");
        }
        if (new_tile_scheduler_metadata_tensor->ndim() != 2
            ...
            throw std::runtime_error(std::string(op_name) + " FlashMLA scheduler metadata shape mismatch.");
        }
