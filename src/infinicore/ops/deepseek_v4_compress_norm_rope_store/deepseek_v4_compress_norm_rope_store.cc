...
INFINICORE_GRAPH_OP_DISPATCHERS_IMPL(DeepseekV4CompressNormRopeStoreKernel);
...
void check_compress_norm_rope_store_shapes(const Tensor &kv,
    ...
    if (kv->dtype() != DataType::BF16 || norm_weight->dtype() != DataType::BF16) {
        throw std::runtime_error(std::string(op_name) + " supports bf16 kv/norm_weight only.");
    }
    if (kv->stride(1) != 1) {
        throw std::runtime_error(std::string(op_name) + " expects contiguous head dimension in kv.");
    }
    if (norm_weight->ndim() != 1 || norm_weight->size(0) != static_cast<size_t>(kHeadDim) || !norm_weight->is_contiguous()) {
        throw std::runtime_error(std::string(op_name) + " expects contiguous norm_weight [512].");
    }
...
DeepseekV4CompressNormRopeStoreKernel::DeepseekV4CompressNormRopeStoreKernel(const Tensor &kv,
                                                                             const Tensor &norm_weight,
                                                                             float epsilon,
                                                                             const Tensor &freqs_cis,
                                                                             const Tensor &positions,
...
void DeepseekV4CompressNormRopeStoreKernel::execute(const Tensor &kv,
                                                    const Tensor &norm_weight,
                                                    float epsilon,
                                                    const Tensor &freqs_cis,
                                                    const Tensor &positions,
                                                    ...
                                                    Tensor kvcache,
                                                    int page_size) {
    INFINICORE_GRAPH_OP_RECORD_OR_RUN(DeepseekV4CompressNormRopeStoreKernel,
                                      kv,
                                      norm_weight,
                                      epsilon,
                                      freqs_cis,
...
namespace deepseek_v4_compress_norm_rope_store_graph_impl {
...
void *plan(const Tensor &kv,
           ...
           Tensor kvcache,
           int page_size) {
    check_compress_norm_rope_store_shapes(kv,
                                          norm_weight,
                                          freqs_cis,
                                          positions,
                                          out_loc,
                                          kvcache,
                                          page_size,
                                          "deepseek_v4_compress_norm_rope_store_kernel_");
...
void run(void *planned_meta) {
    ...
    deepseek_v4_compress_norm_rope_store_native::launch_compress_norm_rope_store(
        planned->kv->data(),
...
#else
    (void)planned_meta;
    throw std::runtime_error("deepseek_v4_compress_norm_rope_store_kernel_ requires a HYGON/NVIDIA build.");
#endif
}
...
namespace deepseek_v4_compress_norm_rope_store_register {
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(DeepseekV4CompressNormRopeStoreKernel,
                                       &deepseek_v4_compress_norm_rope_store_graph_impl::plan,
                                       &deepseek_v4_compress_norm_rope_store_graph_impl::run,
                                       &deepseek_v4_compress_norm_rope_store_graph_impl::cleanup);
} // namespace deepseek_v4_compress_norm_rope_store_register

void deepseek_v4_compress_norm_rope_store_kernel_(const Tensor &kv,
                                                  const Tensor &norm_weight,
                                                  float epsilon,
                                                  const Tensor &freqs_cis,
                                                  const Tensor &positions,
    ...
    check_compress_norm_rope_store_shapes(kv,
                                          ...
                                          page_size,
                                          "deepseek_v4_compress_norm_rope_store_kernel_");
    DeepseekV4CompressNormRopeStoreKernel::execute(kv,
                                                   norm_weight,
                                                   epsilon,
                                                   freqs_cis,
                                                   positions,
...
void deepseek_v4_compress_norm_rope_store_(const Tensor &kv,
                                           const Tensor &norm_weight,
                                           float epsilon,
                                           const Tensor &freqs_cis,
                                           const Tensor &positions,
                                           ...
                                           Tensor kvcache,
                                           int page_size) {
    deepseek_v4_compress_norm_rope_store_kernel_(kv,
                                                 norm_weight,
                                                 epsilon,
                                                 freqs_cis,
                                                 positions,
