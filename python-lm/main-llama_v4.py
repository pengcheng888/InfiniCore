...
def torch_2_infini_ref(model_param: dict):
    ...
    for key, value in model_param.items():
        model_param_infini[key] = infinicore.experimental.torch_2_infini_tensor_ref(
            value
        )
...
def func(Folder):
    ...
    if True:
        for k, v in model_param.items():
            model_param[k] = v.to(device="cuda", dtype=torch.float32)
    ...
    with torch.no_grad():
        ...
        outputs, output_tokens_list = model.generate(
            **input_ids, max_new_tokens=15, device=None
        )
