...
class LlamaMLP(infinicore.nn.Module):
    ...
    def forward(self, x: infinicore.Tensor) -> infinicore.Tensor:
        down_proj = self.down_proj.forward(
            self.act_fn(self.gate_proj.forward(x)) * self.up_proj.forward(x)
        )
        return down_proj
...
class LlamaAttention(infinicore.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # --------------------------------------------------------------------------------------- #
        query_states_infinicore = self.q_proj.forward(hidden_states).view(
            query_hidden_shape
        )  # => [bs, ntok,  num_attention_heads, head_dim]

        key_states_infinicore = self.k_proj.forward(hidden_states).view(
            key_hidden_shape
        )  # => [bs, ntok,  num_key_value_heads, head_dim]

        value_states_infinicore = self.v_proj.forward(hidden_states).view(
            value_hidden_shape
        ...
        query_states = self.rope_infinicore.forward(
            query_states_infinicore, cache_position_infini
        )
        key_states = self.rope_infinicore.forward(
            key_states_infinicore, cache_position_infini
        )
        ...
        # [4, 40, 64]
        att_val = infinicore.attention_lm(
        ...
        attn_output = (
            att_val.view((bs, self.num_attention_heads, ntok, self.head_dim))
            .permute((0, 2, 1, 3))
            .contiguous()
