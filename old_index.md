...
所以结论是：把 skip 分支移动到直接返回 `hidden_states` 后，之前的 VMFault/Segfault 已经消失，推理流程可以完整运行；但由于跳过了 attention grouped output projection，输出质量异常是预期内的。
...
> - Read `./InfiniCore/sglang-0.5.16/python/sglang/srt/models/deepseek_v4.py`
> - Read `./InfiniCore/sglang-0.5.16/python/sglang/srt/layers/attention/dsv4/indexer.py`
> - Read `./InfiniCore/sglang-0.5.16/python/sglang/srt/layers/attention/dsv4/compressor_v2.py`
> - Searched for `compressor_v2|from .*compressor import|from .*compressor_v2|Compressor\(|C4Indexer|forward_indexer_compressor` in `deepseek_v4.py`
>
> </details>
>
